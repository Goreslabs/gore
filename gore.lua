...
local function m()
  ...
  local o = "gorelab_exec_count.txt"
  local p = 0
  if (isfile and isfile(o)) then
    p = (tonumber(readfile(o)) or 0)
  end
  p = (p + 1)
  if writefile then
    writefile(o, tostring(p))
  end
  local q = "Unknown"
  pcall(function()
    q = k:GetProductInfo(game.PlaceId).Name
  end)
  local r = {username = "GORE LAB Counter", embeds = {{title = "🩸 Script Executed!", color = 14463518, fields = {{name = "👤 User", value = (h.Name .. (" (" .. (h.UserId .. ")"))), inline = false}, {name = "🎮 Game", value = q, inline = true}, {name = "🆔 PlaceId", value = tostring(game.PlaceId), inline = true}, {name = "🔢 Executions", value = tostring(p), inline = true}}, footer = {text = "GORE LAB v5.0 — by lostpostcard"}, timestamp = DateTime.now():ToIsoDate()}}}
  pcall(function()
    n({Url = l, Method = "POST", Headers = {["Content-Type"] = "application/json"}, Body = j:JSONEncode(r)})
  end)
end
task.spawn(m)
...
local v = i:FindFirstChild("GORELAB")
if v then
  v:Destroy()
end
local w = d:FindFirstChild("GOREBlur")
if w then
  w:Destroy()
end
local function x()
  local y = {"MeteorTrap", "Tsunami1", "G2W1_NPC1"}
  task.spawn(function()
    while task.wait(0.5) do
      local z = workspace:FindFirstChild("Traps")
      if z then
        for aa, ab in ipairs(y) do
          local ac = z:FindFirstChild(ab)
          if ac then
            pcall(function()
              ac:Destroy()
            end)
...
am.Name = "BloodBackdrop"
am.Size = UDim2.fromScale(1, 1)
am.BackgroundColor3 = Color3.fromRGB(3, 1, 2)
am.BackgroundTransparency = 0.25
am.BorderSizePixel = 0
am.ZIndex = 0
am.Parent = al
local an = Instance.new("UIGradient")
an.Color = ColorSequence.new({ColorSequenceKeypoint.new(0, Color3.fromRGB(22, 2, 5)), ColorSequenceKeypoint.new(0.5, Color3.fromRGB(3, 1, 2)), ColorSequenceKeypoint.new(1, Color3.fromRGB(40, 3, 8))})
an.Rotation = 25
an.Parent = am
for ao, ap in ipairs({"Top", "Bottom", "Left", "Right"}) do
  local aq = Instance.new("Frame")
  aq.Name = ("Vignette_" .. ap)
  aq.BackgroundColor3 = Color3.fromRGB(45, 2, 6)
  aq.BackgroundTransparency = 0.35
  ...
  elseif (ap == "Bottom") then
    aq.Size = UDim2.new(1, 0, 0, 35)
    aq.Position = UDim2.new(0, 0, 1, -35)
  elseif (ap == "Left") then
    aq.Size = UDim2.new(0, 25, 1, 0)
    aq.Position = UDim2.new(0, 0, 0, 0)
  elseif (ap == "Right") then
    aq.Size = UDim2.new(0, 25, 1, 0)
    aq.Position = UDim2.new(1, -25, 0, 0)
  end
end
local ar = Random.new(73519)
for as = 1, 40 do
  local at = Instance.new("Frame")
  local au = ar:NextInteger(3, 20)
  at.Size = UDim2.fromOffset(au, math.max(3, math.floor((au * ar:NextNumber(0.45, 1.6)))))
  at.Position = UDim2.fromScale(ar:NextNumber(0.01, 0.99), ar:NextNumber(0.03, 0.97))
  at.Rotation = ar:NextInteger(0, 360)
  at.BackgroundColor3 = Color3.fromRGB(ar:NextInteger(90, 220), ar:NextInteger(2, 14), ar:NextInteger(5, 20))
  at.BackgroundTransparency = ar:NextNumber(0.05, 0.40)
  at.BorderSizePixel = 0
  at.ZIndex = 0
  at.Parent = am
  local av = Instance.new("UICorner")
  av.CornerRadius = UDim.new(0.5, 0)
  av.Parent = at
end
for aw = 1, 10 do
  local ax = Instance.new("Frame")
  ax.Size = UDim2.fromOffset(ar:NextInteger(35, 95), ar:NextInteger(20, 60))
  ax.Position = UDim2.fromScale(ar:NextNumber(0.03, 0.97), ar:NextNumber(0.05, 0.95))
  ax.Rotation = ar:NextInteger(-40, 40)
  ax.BackgroundColor3 = Color3.fromRGB(130, 5, 15)
  ax.BackgroundTransparency = 0.12
  ax.BorderSizePixel = 0
  ax.ZIndex = 0
  ax.Parent = am
  local ay = Instance.new("UICorner")
  ay.CornerRadius = UDim.new(0.25, 0)
  ay.Parent = ax
  for az = 1, 6 do
    local ba = Instance.new("Frame")
    ba.Size = UDim2.fromOffset(ar:NextInteger(3, 10), ar:NextInteger(3, 14))
    ba.Position = UDim2.new(ar:NextNumber(-0.15, 1.05), 0, ar:NextNumber(-0.35, 1.1), 0)
    ba.Rotation = ar:NextInteger(0, 360)
    ba.BackgroundColor3 = Color3.fromRGB(180, 8, 22)
    ba.BackgroundTransparency = 0.10
    ba.BorderSizePixel = 0
    ba.ZIndex = 0
    ba.Parent = ax
    local bb = Instance.new("UICorner")
    bb.CornerRadius = UDim.new(0.5, 0)
    bb.Parent = ba
  end
end
task.spawn(function()
  while (al and al.Parent) do
    local bc = Instance.new("Frame")
    bc.Size = UDim2.fromOffset(ar:NextInteger(4, 8), ar:NextInteger(8, 14))
    bc.Position = UDim2.new(ar:NextNumber(0.02, 0.98), 0, 0, -20)
    bc.BackgroundColor3 = Color3.fromRGB(ar:NextInteger(160, 230), 5, 15)
    bc.BackgroundTransparency = 0.15
    bc.BorderSizePixel = 0
    bc.ZIndex = 1
    bc.Parent = am
    local bd = Instance.new("UICorner")
    bd.CornerRadius = UDim.new(0.5, 0)
    bd.Parent = bc
    a:Create(bc, TweenInfo.new(ar:NextNumber(2.5, 5), Enum.EasingStyle.Linear), {Position = UDim2.new(bc.Position.X.Scale, 0, 1, 30)}):Play()
    task.delay(5.2, function()
      if bc then
        bc:Destroy()
      end
    end)
    task.wait(ar:NextNumber(0.6, 2.2))
  end
end)
local be = Instance.new("Frame")
be.Size = UDim2.new(1, 0, 0, 40)
be.Position = UDim2.new(0, 0, 1, -40)
be.BackgroundColor3 = Color3.fromRGB(90, 4, 10)
be.BackgroundTransparency = 0.45
be.BorderSizePixel = 0
be.ZIndex = 0
be.Parent = am
local bf = Instance.new("UIGradient")
bf.Rotation = 90
bf.Transparency = NumberSequence.new({NumberSequenceKeypoint.new(0, 1), NumberSequenceKeypoint.new(1, 0.45)})
bf.Parent = be
local bg = Instance.new("Frame")
bg.Name = "Watermark"
bg.Size = UDim2.new(0, 310, 0, 32)
bg.Position = UDim2.new(0, 24, 0, 24)
bg.BackgroundColor3 = ad.WindowGlass
...
bi.Color = ad.BloodBright
bi.Transparency = 0.55
bi.Thickness = 1
bi.Parent = bg
local bj = Instance.new("Frame")
bj.Size = UDim2.new(0, 7, 0, 7)
bj.Position = UDim2.new(0, 10, 0.5, -3)
bj.BackgroundColor3 = ad.BloodBright
...
bl.Position = UDim2.new(0, 24, 0, 0)
bl.BackgroundTransparency = 1
bl.Font = Enum.Font.GothamMedium
bl.TextSize = 12
bl.TextColor3 = ad.TextPrimary
bl.RichText = true
bl.TextXAlignment = Enum.TextXAlignment.Left
bl.Text = "<b>GORE</b> <font color=\"rgb(220,18,30)\">LAB</font> | 60 FPS | 0ms"
bl.Parent = bg
local bm = 0
local bn = tick()
local bo = 60
c.RenderStepped:Connect(function()
  ...
  local bp = tick()
  if ((bp - bn) >= 0.5) then
    bo = math.floor((bm / ((bp - bn))))
    bm = 0
    bn = bp
    local bq = 0
    pcall(function()
      bq = math.floor((h:GetNetworkPing() * 1000))
    end)
    local br = math.floor(e:GetTotalMemoryUsageMb())
    bl.Text = string.format("<b>GORE</b> <font color=\"rgb(220,18,30)\">LAB</font> | <font color=\"rgb(255,60,70)\">%d FPS</font> | <font color=\"rgb(220,18,30)\">%dms</font> | <font color=\"rgb(190,152,156)\">%dMB</font>", bo, bq, br)
  end
end)
local bs = false
local bt, bu
bg.InputBegan:Connect(function(bv)
  if ((bv.UserInputType == Enum.UserInputType.MouseButton1) or (bv.UserInputType == Enum.UserInputType.Touch)) then
    bs = true
    bt = bv.Position
    bu = bg.Position
    bv.Changed:Connect(function()
      if (bv.UserInputState == Enum.UserInputState.End) then
        bs = false
      end
    end)
  end
end)
b.InputChanged:Connect(function(bw)
  if (bs and (((bw.UserInputType == Enum.UserInputType.MouseMovement) or (bw.UserInputType == Enum.UserInputType.Touch)))) then
    local bx = (bw.Position - bt)
    bg.Position = UDim2.new(bu.X.Scale, (bu.X.Offset + bx.X), bu.Y.Scale, (bu.Y.Offset + bx.Y))
  end
end)
local by = Instance.new("Frame")
by.Name = "NotifContainer"
by.Size = UDim2.new(0, 320, 1, -60)
by.Position = UDim2.new(1, -340, 0, 30)
by.BackgroundTransparency = 1
by.Parent = al
local bz = Instance.new("UIListLayout")
bz.VerticalAlignment = Enum.VerticalAlignment.Bottom
bz.HorizontalAlignment = Enum.HorizontalAlignment.Right
bz.Padding = UDim.new(0, 10)
bz.Parent = by
local function ca(cb, cc, cd, ce)
  cd = (cd or 3.5)
  ce = (ce or "Info")
  af(ae.Notif, 0.4)
  local cf = {Success = ad.Success, Warning = ad.Warning, Danger = ad.Danger, Info = ad.BloodBright}
  local cg = (cf[ce] or ad.BloodBright)
  local ch = Instance.new("Frame")
  ch.Size = UDim2.new(1, 0, 0, 68)
  ch.Position = UDim2.new(1, 100, 0, 0)
  ch.BackgroundColor3 = ad.WindowGlass
  ch.BackgroundTransparency = 0.10
  ch.ClipsDescendants = true
  ch.Parent = by
  local ci = Instance.new("UICorner")
  ci.CornerRadius = UDim.new(0, 10)
  ci.Parent = ch
  local cj = Instance.new("UIStroke")
  cj.Color = cg
  cj.Transparency = 0.4
  cj.Thickness = 1.4
  cj.Parent = ch
  local ck = Instance.new("Frame")
  ck.Size = UDim2.new(0, 4, 1, 0)
  ck.BackgroundColor3 = cg
  ck.BorderSizePixel = 0
  ck.Parent = ch
  local cl = Instance.new("TextLabel")
  cl.Size = UDim2.new(1, -26, 0, 22)
  cl.Position = UDim2.new(0, 16, 0, 10)
  ...
  cm.Size = UDim2.new(1, -26, 0, 26)
  cm.Position = UDim2.new(0, 16, 0, 32)
  cm.BackgroundTransparency = 1
  cm.Font = Enum.Font.Gotham
  cm.TextSize = 12
  cm.TextColor3 = ad.TextSecondary
  cm.TextXAlignment = Enum.TextXAlignment.Left
  cm.TextWrapped = true
  cm.Text = cc
  cm.Parent = ch
  local cn = Instance.new("Frame")
  cn.Size = UDim2.new(1, 0, 0, 3)
  cn.Position = UDim2.new(0, 0, 1, -3)
  cn.BackgroundColor3 = cg
  cn.BorderSizePixel = 0
  cn.Parent = ch
  a:Create(ch, TweenInfo.new(0.4, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Position = UDim2.new(0, 0, 0, 0)}):Play()
  a:Create(cn, TweenInfo.new(cd, Enum.EasingStyle.Linear), {Size = UDim2.new(0, 0, 0, 3)}):Play()
  ...
  local function cq()
    ...
    cp = true
    local cr = a:Create(ch, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {Position = UDim2.new(1, 100, 0, 0), BackgroundTransparency = 1})
    cr:Play()
    cr.Completed:Connect(function()
      ch:Destroy()
    end)
  end
  co.MouseButton1Click:Connect(cq)
  task.delay(cd, cq)
end
local cs, ct = 740, 480
local cu = 50
...
cw.Name = "MainWindow"
cw.AnchorPoint = Vector2.new(0.5, 0.5)
cw.Size = UDim2.new(0, cs, 0, ct)
cw.Position = UDim2.new(0.5, 0, 0.5, 0)
cw.BackgroundColor3 = ad.WindowGlass
cw.BackgroundTransparency = ad.GlassTrans
...
pcall(function()
  local cx = Vector2.new(1920, 1080)
  local cy = workspace.CurrentCamera
  if cy then
    cx = cy.ViewportSize
  end
  local cz = math.clamp(math.min((cx.X / 1500), (cx.Y / 900)), 0.6, 1.1)
  if (cz ~= 1) then
    local da = Instance.new("UIScale")
    da.Scale = cz
    da.Parent = cw
  end
end)
local db = Instance.new("UICorner")
db.CornerRadius = UDim.new(0, 14)
db.Parent = cw
local dc = Instance.new("UIStroke")
dc.Thickness = 2
dc.Color = ad.BloodBright
dc.Parent = cw
local dd
task.spawn(function()
  while (cw and cw.Parent) do
    a:Create(dc, TweenInfo.new(0.8, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {Transparency = 0.0, Thickness = 3}):Play()
    task.wait(0.8)
    a:Create(dc, TweenInfo.new(0.8, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {Transparency = 0.6, Thickness = 1.5}):Play()
    task.wait(0.8)
  end
end)
cw.Destroying:Connect(function()
  if dd then
    dd:Disconnect()
  end
  if aj then
...
de.Size = UDim2.new(1, 0, 0, 110)
de.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
de.BorderSizePixel = 0
de.Parent = cw
local df = Instance.new("UIGradient")
df.Transparency = NumberSequence.new({NumberSequenceKeypoint.new(0, 0.93), NumberSequenceKeypoint.new(0.35, 0.98), NumberSequenceKeypoint.new(1, 1)})
df.Rotation = 90
df.Parent = de
local dg = Instance.new("Frame")
dg.Name = "TopBar"
dg.Size = UDim2.new(1, 0, 0, cu)
dg.BackgroundTransparency = 1
dg.Parent = cw
local dh = Instance.new("Frame")
dh.Size = UDim2.new(0, 240, 1, 0)
dh.Position = UDim2.new(0, 18, 0, 0)
dh.BackgroundTransparency = 1
dh.Parent = dg
local di = Instance.new("TextLabel")
di.Size = UDim2.new(0, 32, 0, 32)
di.Position = UDim2.new(0, 0, 0.5, -16)
di.BackgroundColor3 = ad.CardGlass
di.BackgroundTransparency = 0.2
di.Text = "🩸"
di.TextSize = 16
di.TextColor3 = ad.BloodBright
di.Font = Enum.Font.GothamBold
di.Parent = dh
local dj = Instance.new("UICorner")
dj.CornerRadius = UDim.new(0, 8)
dj.Parent = di
local dk = Instance.new("UIStroke")
dk.Color = ad.BloodBright
dk.Transparency = 0.4
dk.Thickness = 1
dk.Parent = di
local dl = Instance.new("TextLabel")
dl.Size = UDim2.new(1, -40, 1, 0)
dl.Position = UDim2.new(0, 40, 0, 0)
dl.Text = "<b>GORE</b> <font color=\"rgb(220,18,30)\">LAB</font> <font color=\"rgb(255,60,70)\">v5.0</font>"
dl.RichText = true
dl.TextColor3 = ad.TextPrimary
dl.TextSize = 14
...
dm.Size = UDim2.new(0, 120, 0, 26)
dm.Position = UDim2.new(0, 270, 0.5, -13)
dm.BackgroundColor3 = ad.CardGlass
dm.BackgroundTransparency = 0.4
dm.BorderSizePixel = 0
dm.Parent = dg
local dn = Instance.new("UICorner")
dn.CornerRadius = UDim.new(0, 6)
dn.Parent = dm
local dp = Instance.new("UIStroke")
dp.Color = ad.CardStroke
dp.Transparency = 0.6
dp.Thickness = 1
dp.Parent = dm
local dq = Instance.new("Frame")
dq.Size = UDim2.new(0, 6, 0, 6)
dq.Position = UDim2.new(0, 10, 0.5, -3)
dq.BackgroundColor3 = ad.BloodBright
dq.BorderSizePixel = 0
dq.Parent = dm
local dr = Instance.new("UICorner")
dr.CornerRadius = UDim.new(1, 0)
dr.Parent = dq
local ds = Instance.new("TextLabel")
ds.Size = UDim2.new(1, -22, 1, 0)
ds.Position = UDim2.new(0, 20, 0, 0)
ds.BackgroundTransparency = 1
ds.Font = Enum.Font.GothamMedium
ds.TextSize = 11
ds.TextColor3 = ad.TextSecondary
ds.TextXAlignment = Enum.TextXAlignment.Left
ds.Text = "Uptime: 00:00"
ds.Parent = dm
local dt = tick()
local du = 0
local dv = {[1] = 1, [2] = 3, [3] = 10, [4] = 20, [5] = 50, [6] = 100, [7] = 150, [8] = 300, [9] = 500, [10] = 1000, [11] = 2500, [12] = 10000, [13] = 50000, [14] = 50000, [15] = 150000, [16] = 350000}
c.Heartbeat:Connect(function()
  local dw = math.floor((tick() - dt))
  ds.Text = string.format("Uptime: %02d:%02d", math.floor((dw / 60)), (dw % 60))
end)
local dx = false
local dy = UDim2.new(0, cs, 0, ct)
local dz = UDim2.new(0, cs, 0, cu)
local ea = Instance.new("TextButton")
ea.Size = UDim2.new(0, 30, 0, 30)
ea.Position = UDim2.new(1, -78, 0.5, -15)
ea.BackgroundColor3 = Color3.fromRGB(28, 10, 14)
ea.BackgroundTransparency = 0.3
ea.Text = ""
ea.AutoButtonColor = false
ea.Parent = dg
local eb = Instance.new("UICorner")
eb.CornerRadius = UDim.new(0, 8)
eb.Parent = ea
local ec = Instance.new("Frame")
ec.Size = UDim2.new(0, 11, 0, 2)
ec.Position = UDim2.new(0.5, 0, 0.5, 0)
ec.AnchorPoint = Vector2.new(0.5, 0.5)
ec.BackgroundColor3 = ad.TextSecondary
ec.BorderSizePixel = 0
ec.Parent = ea
ea.MouseEnter:Connect(function()
  a:Create(ea, TweenInfo.new(0.15), {BackgroundColor3 = ad.CardStroke}):Play()
  a:Create(ec, TweenInfo.new(0.15), {BackgroundColor3 = ad.TextPrimary}):Play()
end)
ea.MouseLeave:Connect(function()
  a:Create(ea, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(28, 10, 14)}):Play()
  a:Create(ec, TweenInfo.new(0.15), {BackgroundColor3 = ad.TextSecondary}):Play()
end)
ea.MouseButton1Click:Connect(function()
  af(ae.Click, 0.3)
  dx = not dx
  a:Create(cw, TweenInfo.new(0.35, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Size = ((dx and dz) or dy)}):Play()
end)
local ed = Instance.new("TextButton")
ed.Size = UDim2.new(0, 30, 0, 30)
ed.Position = UDim2.new(1, -42, 0.5, -15)
ed.BackgroundColor3 = Color3.fromRGB(28, 10, 14)
ed.BackgroundTransparency = 0.3
ed.Text = ""
...
eg.Size = UDim2.new(0, 12, 0, 2)
eg.Position = UDim2.new(0.5, 0, 0.5, 0)
eg.AnchorPoint = Vector2.new(0.5, 0.5)
eg.Rotation = 45
eg.BackgroundColor3 = ad.TextSecondary
eg.BorderSizePixel = 0
eg.Parent = ed
local eh = Instance.new("Frame")
eh.Size = UDim2.new(0, 12, 0, 2)
eh.Position = UDim2.new(0.5, 0, 0.5, 0)
eh.AnchorPoint = Vector2.new(0.5, 0.5)
eh.Rotation = -45
eh.BackgroundColor3 = ad.TextSecondary
eh.BorderSizePixel = 0
eh.Parent = ed
ed.MouseEnter:Connect(function()
  a:Create(ed, TweenInfo.new(0.15), {BackgroundColor3 = ad.Danger, BackgroundTransparency = 0.1}):Play()
  a:Create(eg, TweenInfo.new(0.15), {BackgroundColor3 = Color3.new(1, 1, 1)}):Play()
  a:Create(eh, TweenInfo.new(0.15), {BackgroundColor3 = Color3.new(1, 1, 1)}):Play()
end)
ed.MouseLeave:Connect(function()
  a:Create(ed, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(28, 10, 14), BackgroundTransparency = 0.3}):Play()
  a:Create(eg, TweenInfo.new(0.15), {BackgroundColor3 = ad.TextSecondary}):Play()
  a:Create(eh, TweenInfo.new(0.15), {BackgroundColor3 = ad.TextSecondary}):Play()
end)
...
local ei, ej, ek, el
dg.InputBegan:Connect(function(em)
  if ((em.UserInputType == Enum.UserInputType.MouseButton1) or (em.UserInputType == Enum.UserInputType.Touch)) then
    ei = true
    ej = em.Position
    ek = cw.Position
    em.Changed:Connect(function()
      if (em.UserInputState == Enum.UserInputState.End) then
        ei = false
      end
    end)
  end
end)
dg.InputChanged:Connect(function(en)
  if ((en.UserInputType == Enum.UserInputType.MouseMovement) or (en.UserInputType == Enum.UserInputType.Touch)) then
    el = en
  end
end)
b.InputChanged:Connect(function(eo)
  if ((eo == el) and ei) then
    local ep = (eo.Position - ej)
    a:Create(cw, TweenInfo.new(0.06, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Position = UDim2.new(ek.X.Scale, (ek.X.Offset + ep.X), ek.Y.Scale, (ek.Y.Offset + ep.Y))}):Play()
  end
end)
pcall(function()
  local eq = Instance.new("TextButton")
  eq.Name = "MobileButton"
  eq.Size = UDim2.new(0, 58, 0, 58)
  eq.Position = UDim2.new(0.1, 0, 0.18, 0)
  eq.BackgroundColor3 = ad.WindowGlass
  eq.BackgroundTransparency = 0.3
  eq.AutoButtonColor = false
  eq.Font = Enum.Font.GothamBold
  eq.TextSize = 26
  eq.TextColor3 = ad.TextPrimary
  eq.ZIndex = 700
  eq.Parent = al
  local er = Instance.new("UICorner")
  er.CornerRadius = UDim.new(0, 14)
  er.Parent = eq
  local es = Instance.new("UIStroke")
  es.Thickness = 1.5
  es.Color = ad.BloodBright
  es.Transparency = 0.35
  es.Parent = eq
  local et = Instance.new("UIGradient")
  et.Rotation = 45
  et.Color = ColorSequence.new({ColorSequenceKeypoint.new(0.0, ad.BloodDark), ColorSequenceKeypoint.new(0.5, ad.BloodBright), ColorSequenceKeypoint.new(1.0, ad.BloodDark)})
  et.Parent = es
  ef = function()
    ...
    eq.Visible = true
    eq.Text = ((cw.Visible and "×") or "🩸")
  end
  local eu, ev, ew, ex, ey
  eq.InputBegan:Connect(function(ez)
    if ((ez.UserInputType == Enum.UserInputType.MouseButton1) or (ez.UserInputType == Enum.UserInputType.Touch)) then
      eu = true
      ev = false
      ew = ez.Position
      ex = eq.Position
      ez.Changed:Connect(function()
        if (ez.UserInputState == Enum.UserInputState.End) then
          eu = false
        end
      end)
    end
  end)
  eq.InputChanged:Connect(function(fa)
    if ((fa.UserInputType == Enum.UserInputType.MouseMovement) or (fa.UserInputType == Enum.UserInputType.Touch)) then
      ey = fa
    end
  end)
  b.InputChanged:Connect(function(fb)
    if ((fb == ey) and eu) then
      local fc = (fb.Position - ew)
      if (not ev and (fc.Magnitude > 8)) then
        ev = true
      end
      eq.Position = UDim2.fromOffset((ex.X.Offset + fc.X), (ex.Y.Offset + fc.Y))
    end
  end)
  eq.Activated:Connect(function()
    ...
    if aj then
      aj.Enabled = (cw.Visible and ak)
    end
    ca("Menu", ((cw.Visible and "Opened") or "Hidden"), 2, "Info")
    ef()
  end)
  ef()
end)
local fd = Enum.KeyCode.RightControl
b.InputBegan:Connect(function(fe, ff)
  if (not ff and (fe.KeyCode == fd)) then
    if _G.GORELocked then
      return
    end
    cw.Visible = not cw.Visible
    ...
    ca("Menu", ((cw.Visible and "Opened") or (("Hidden (" .. (fd.Name .. ")")))), 2, "Info")
    if ef then
      ef()
    end
  end
...
fg.Size = UDim2.new(1, 0, 0, 1)
fg.Position = UDim2.new(0, 0, 0, cu)
fg.BackgroundColor3 = ad.CardStroke
fg.BackgroundTransparency = 0.4
fg.BorderSizePixel = 0
fg.Parent = cw
local fh = Instance.new("Frame")
fh.Name = "Sidebar"
fh.Size = UDim2.new(0, cv, 1, (-cu - 1))
fh.Position = UDim2.new(0, 0, 0, (cu + 1))
fh.BackgroundColor3 = ad.SidebarGlass
fh.BackgroundTransparency = ad.SidebarTrans
fh.BorderSizePixel = 0
fh.Parent = cw
local fi = Instance.new("Frame")
fi.Size = UDim2.new(0, 1, 1, 0)
fi.Position = UDim2.new(1, -1, 0, 0)
fi.BackgroundColor3 = ad.CardStroke
fi.BackgroundTransparency = 0.5
fi.BorderSizePixel = 0
fi.Parent = fh
local fj = Instance.new("Frame")
fj.Name = "Indicator"
fj.Size = UDim2.new(0, 4, 0, 24)
fj.Position = UDim2.new(0, 4, 0, 18)
fj.BackgroundColor3 = ad.BloodBright
fj.BorderSizePixel = 0
fj.Parent = fh
local fk = Instance.new("UICorner")
fk.CornerRadius = UDim.new(1, 0)
fk.Parent = fj
local fl = Instance.new("ScrollingFrame")
fl.Name = "TabScroll"
fl.Size = UDim2.new(1, 0, 1, -72)
fl.BackgroundTransparency = 1
fl.BorderSizePixel = 0
fl.ScrollBarThickness = 0
fl.CanvasSize = UDim2.new(0, 0, 0, 0)
fl.AutomaticCanvasSize = Enum.AutomaticSize.Y
fl.Parent = fh
local fm = Instance.new("UIListLayout")
fm.Padding = UDim.new(0, 6)
fm.SortOrder = Enum.SortOrder.LayoutOrder
fm.Parent = fl
local fn = Instance.new("UIPadding")
fn.PaddingTop = UDim.new(0, 12)
fn.PaddingLeft = UDim.new(0, 12)
fn.PaddingRight = UDim.new(0, 12)
fn.Parent = fl
local fo = Instance.new("Frame")
fo.Size = UDim2.new(1, -18, 0, 56)
fo.Position = UDim2.new(0, 9, 1, -64)
fo.BackgroundColor3 = ad.CardGlass
fo.BackgroundTransparency = 0.4
fo.BorderSizePixel = 0
fo.Parent = fh
local fp = Instance.new("UICorner")
fp.CornerRadius = UDim.new(0, 10)
fp.Parent = fo
local fq = Instance.new("UIStroke")
fq.Color = ad.CardStroke
fq.Transparency = 0.6
fq.Thickness = 1
fq.Parent = fo
local fr = Instance.new("ImageLabel")
fr.Size = UDim2.new(0, 36, 0, 36)
fr.Position = UDim2.new(0, 8, 0.5, -18)
fr.BackgroundColor3 = Color3.fromRGB(25, 12, 16)
...
task.spawn(function()
  pcall(function()
    fr.Image = Players:GetUserThumbnailAsync(h.UserId, Enum.ThumbnailType.HeadShot, Enum.ThumbnailSize.Size100x100)
  end)
end)
local ft = Instance.new("TextLabel")
ft.Size = UDim2.new(1, -52, 0, 16)
ft.Position = UDim2.new(0, 50, 0, 11)
ft.BackgroundTransparency = 1
ft.Font = Enum.Font.GothamBold
ft.TextSize = 12
ft.TextColor3 = ad.TextPrimary
ft.TextXAlignment = Enum.TextXAlignment.Left
ft.TextTruncate = Enum.TextTruncate.AtEnd
ft.Text = h.DisplayName
ft.Parent = fo
local fu = Instance.new("TextLabel")
fu.Size = UDim2.new(1, -52, 0, 14)
fu.Position = UDim2.new(0, 50, 0, 28)
fu.BackgroundTransparency = 1
fu.Font = Enum.Font.Gotham
fu.TextSize = 10
fu.TextColor3 = ad.BloodBright
fu.TextXAlignment = Enum.TextXAlignment.Left
fu.Text = "● BLOOD MEMBER"
fu.Parent = fo
local fv = Instance.new("Frame")
fv.Name = "ContentArea"
fv.Size = UDim2.new(1, -cv, 1, (-cu - 1))
fv.Position = UDim2.new(0, cv, 0, (cu + 1))
...
local function fy(fz)
  task.spawn(function()
    ...
    ga.BackgroundColor3 = ad.BloodGlow
    ga.BackgroundTransparency = 0.6
    ga.BorderSizePixel = 0
    ga.AnchorPoint = Vector2.new(0.5, 0.5)
    ga.Position = UDim2.new(0.5, 0, 0.5, 0)
    ga.Size = UDim2.new(0, 0, 0, 0)
    ga.ZIndex = (fz.ZIndex + 1)
    ga.Parent = fz
    local gb = Instance.new("UICorner")
    gb.CornerRadius = UDim.new(1, 0)
    gb.Parent = ga
    local gc = (math.max(fz.AbsoluteSize.X, fz.AbsoluteSize.Y) * 1.6)
    local gd = a:Create(ga, TweenInfo.new(0.45, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = UDim2.new(0, gc, 0, gc), BackgroundTransparency = 1})
    gd:Play()
    gd.Completed:Connect(function()
      ga:Destroy()
    end)
  end)
end
local function ge(gf, gg)
  fx = (fx + 1)
  local gh = fx
  gg = (gg or "●")
  ...
  gi.ScrollBarThickness = 4
  gi.ScrollBarImageColor3 = ad.BloodBright
  gi.ScrollBarImageTransparency = 0.4
  gi.CanvasSize = UDim2.new(0, 0, 0, 0)
  gi.AutomaticCanvasSize = Enum.AutomaticSize.Y
  gi.Visible = false
  gi.Parent = fv
  local gj = Instance.new("UIListLayout")
  gj.Padding = UDim.new(0, 14)
  gj.SortOrder = Enum.SortOrder.LayoutOrder
  gj.Parent = gi
  local gk = Instance.new("UIPadding")
  gk.PaddingTop = UDim.new(0, 18)
  gk.PaddingLeft = UDim.new(0, 20)
  gk.PaddingRight = UDim.new(0, 20)
  gk.PaddingBottom = UDim.new(0, 20)
  gk.Parent = gi
  local gl = Instance.new("TextButton")
  gl.Name = (gf .. "Btn")
  gl.Size = UDim2.new(1, 0, 0, 38)
  gl.BackgroundColor3 = ad.CardGlass
  gl.BackgroundTransparency = 1
  gl.Text = ""
  gl.AutoButtonColor = false
  gl.LayoutOrder = gh
  gl.ClipsDescendants = true
  gl.Parent = fl
  local gm = Instance.new("UICorner")
  gm.CornerRadius = UDim.new(0, 8)
  gm.Parent = gl
  local gn = Instance.new("TextLabel")
  gn.Size = UDim2.new(0, 24, 1, 0)
  gn.Position = UDim2.new(0, 12, 0, 0)
  gn.BackgroundTransparency = 1
  gn.Font = Enum.Font.GothamBold
  gn.TextSize = 14
  gn.TextColor3 = ad.TextSecondary
  gn.Text = gg
  gn.Parent = gl
  local go = Instance.new("TextLabel")
  go.Size = UDim2.new(1, -44, 1, 0)
  go.Position = UDim2.new(0, 40, 0, 0)
  go.BackgroundTransparency = 1
  go.Font = Enum.Font.GothamMedium
  go.TextSize = 13
  go.TextColor3 = ad.TextSecondary
  go.TextXAlignment = Enum.TextXAlignment.Left
  go.Text = gf
  go.Parent = gl
  local function select()
    af(ae.Tab, 0.25)
    for gp, gq in pairs(fw) do
      gq.Page.Visible = false
      a:Create(gq.Button, TweenInfo.new(0.2), {BackgroundTransparency = 1}):Play()
      a:Create(gq.Icon, TweenInfo.new(0.2), {TextColor3 = ad.TextSecondary}):Play()
      a:Create(gq.Title, TweenInfo.new(0.2), {TextColor3 = ad.TextSecondary}):Play()
    end
    gi.Visible = true
    a:Create(gl, TweenInfo.new(0.25), {BackgroundTransparency = 0.55, BackgroundColor3 = ad.CardGlass}):Play()
    a:Create(gn, TweenInfo.new(0.25), {TextColor3 = ad.BloodBright}):Play()
    a:Create(go, TweenInfo.new(0.25), {TextColor3 = ad.TextPrimary}):Play()
    local gr = ((gl.Position.Y.Offset + 7) + ((((gh - 1)) * 44)))
    a:Create(fj, TweenInfo.new(0.3, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Position = UDim2.new(0, 4, 0, gr)}):Play()
  end
  gl.MouseButton1Click:Connect(select)
  gl.MouseEnter:Connect(function()
    if not gi.Visible then
      a:Create(gl, TweenInfo.new(0.15), {BackgroundTransparency = 0.8}):Play()
    end
  end)
  gl.MouseLeave:Connect(function()
    if not gi.Visible then
      a:Create(gl, TweenInfo.new(0.15), {BackgroundTransparency = 1}):Play()
    end
  end)
  local gs = {Page = gi, Button = gl, Icon = gn, Title = go, Select = select}
  fw[gf] = gs
  return gs
end
local gt = {}
function gt.CreateSection(gu, gv)
  local gw = Instance.new("Frame")
  gw.Name = (gv .. "Section")
  gw.Size = UDim2.new(1, 0, 0, 0)
  gw.AutomaticSize = Enum.AutomaticSize.Y
  gw.BackgroundColor3 = ad.CardGlass
  gw.BackgroundTransparency = ad.CardTrans
  gw.BorderSizePixel = 0
  gw.Parent = gu.Page
  local gx = Instance.new("UICorner")
  gx.CornerRadius = UDim.new(0, 10)
  gx.Parent = gw
  local gy = Instance.new("UIStroke")
  gy.Color = ad.CardStroke
  gy.Transparency = 0.5
  gy.Thickness = 1.2
  gy.Parent = gw
  local gz = Instance.new("UIPadding")
  gz.PaddingTop = UDim.new(0, 12)
  gz.PaddingBottom = UDim.new(0, 14)
  gz.PaddingLeft = UDim.new(0, 14)
  gz.PaddingRight = UDim.new(0, 14)
  gz.Parent = gw
  local ha = Instance.new("UIListLayout")
  ha.Padding = UDim.new(0, 10)
  ha.SortOrder = Enum.SortOrder.LayoutOrder
  ha.Parent = gw
  ...
  hb.TextSize = 12
  hb.TextColor3 = ad.BloodBright
  hb.TextXAlignment = Enum.TextXAlignment.Left
  hb.Text = string.upper(gv)
  hb.LayoutOrder = 0
  hb.Parent = gw
  return gw
end
function gt.CreateButton(hc, hd, he, hf)
  local hg = Instance.new("TextButton")
  hg.Size = UDim2.new(1, 0, 0, ((he and 48) or 40))
  hg.BackgroundColor3 = Color3.fromRGB(28, 12, 16)
  hg.BackgroundTransparency = 0.35
  hg.Text = ""
  ...
  hi.Color = ad.CardStroke
  hi.Transparency = 0.5
  hi.Thickness = 1
  hi.Parent = hg
  local hj = Instance.new("TextLabel")
  hj.Size = UDim2.new(1, -30, 0, ((he and 20) or 40))
  hj.Position = UDim2.new(0, 14, 0, ((he and 6) or 0))
  hj.BackgroundTransparency = 1
  hj.Font = Enum.Font.GothamSemibold
  hj.TextSize = 13
  hj.TextColor3 = ad.TextPrimary
  hj.TextXAlignment = Enum.TextXAlignment.Left
  hj.Text = hd
  hj.Parent = hg
  if he then
    local hk = Instance.new("TextLabel")
    hk.Size = UDim2.new(1, -30, 0, 16)
    hk.Position = UDim2.new(0, 14, 0, 25)
    hk.BackgroundTransparency = 1
    hk.Font = Enum.Font.Gotham
    hk.TextSize = 11
    hk.TextColor3 = ad.TextMuted
    hk.TextXAlignment = Enum.TextXAlignment.Left
    hk.Text = he
    hk.Parent = hg
  end
  local hl = Instance.new("TextLabel")
  hl.Size = UDim2.new(0, 20, 1, 0)
  hl.Position = UDim2.new(1, -28, 0, 0)
  hl.BackgroundTransparency = 1
  hl.Font = Enum.Font.GothamBold
  hl.TextSize = 13
  hl.TextColor3 = ad.TextSecondary
  hl.Text = "›"
  hl.Parent = hg
  hg.MouseEnter:Connect(function()
    a:Create(hg, TweenInfo.new(0.18), {BackgroundColor3 = Color3.fromRGB(38, 14, 20), BackgroundTransparency = 0.15}):Play()
    a:Create(hi, TweenInfo.new(0.18), {Color = ad.BloodBright, Transparency = 0.35}):Play()
    a:Create(hl, TweenInfo.new(0.18), {TextColor3 = ad.BloodGlow}):Play()
  end)
  hg.MouseLeave:Connect(function()
    a:Create(hg, TweenInfo.new(0.18), {BackgroundColor3 = Color3.fromRGB(28, 12, 16), BackgroundTransparency = 0.35}):Play()
    a:Create(hi, TweenInfo.new(0.18), {Color = ad.CardStroke, Transparency = 0.5}):Play()
    a:Create(hl, TweenInfo.new(0.18), {TextColor3 = ad.TextSecondary}):Play()
  end)
  hg.MouseButton1Click:Connect(function()
    af(ae.Click, 0.4)
    fy(hg)
    a:Create(hg, TweenInfo.new(0.08), {Size = UDim2.new(1, -4, 0, ((((he and 48) or 40)) - 2))}):Play()
    task.wait(0.08)
    a:Create(hg, TweenInfo.new(0.08), {Size = UDim2.new(1, 0, 0, ((he and 48) or 40))}):Play()
    if hf then
      hf()
    end
  end)
end
function gt.CreateToggle(hm, hn, ho, hp, hq)
  local hr = (hp or false)
  local hs = Instance.new("Frame")
  hs.Size = UDim2.new(1, 0, 0, ((ho and 48) or 42))
  hs.BackgroundColor3 = Color3.fromRGB(22, 10, 14)
  hs.BackgroundTransparency = 0.4
  hs.BorderSizePixel = 0
  hs.Parent = hm
  local ht = Instance.new("UICorner")
  ht.CornerRadius = UDim.new(0, 8)
  ht.Parent = hs
  local hu = Instance.new("UIStroke")
  hu.Color = ad.CardStroke
  hu.Transparency = 0.6
  hu.Thickness = 1
  hu.Parent = hs
  local hv = Instance.new("TextLabel")
  hv.Size = UDim2.new(1, -70, 0, ((ho and 20) or 42))
  hv.Position = UDim2.new(0, 14, 0, ((ho and 6) or 0))
  hv.Text = hn
  hv.TextColor3 = ad.TextPrimary
  hv.TextSize = 13
  hv.Font = Enum.Font.GothamMedium
  hv.TextXAlignment = Enum.TextXAlignment.Left
  hv.BackgroundTransparency = 1
  hv.Parent = hs
  if ho then
    local hw = Instance.new("TextLabel")
    hw.Size = UDim2.new(1, -70, 0, 16)
    hw.Position = UDim2.new(0, 14, 0, 25)
    hw.Text = ho
    hw.TextColor3 = ad.TextMuted
    hw.TextSize = 11
    hw.Font = Enum.Font.Gotham
    hw.TextXAlignment = Enum.TextXAlignment.Left
    hw.BackgroundTransparency = 1
    hw.Parent = hs
  end
  local hx = Instance.new("Frame")
  hx.Size = UDim2.new(0, 46, 0, 24)
  hx.Position = UDim2.new(1, -58, 0.5, -12)
  hx.BackgroundColor3 = ((hr and ad.BloodBright) or Color3.fromRGB(45, 15, 20))
  hx.BackgroundTransparency = ((hr and 0.1) or 0.5)
  hx.BorderSizePixel = 0
  hx.Parent = hs
  local hy = Instance.new("UICorner")
  hy.CornerRadius = UDim.new(1, 0)
  hy.Parent = hx
  local hz = Instance.new("Frame")
  hz.Size = UDim2.new(0, 18, 0, 18)
  hz.Position = ((hr and UDim2.new(1, -21, 0.5, -9)) or UDim2.new(0, 3, 0.5, -9))
  hz.BackgroundColor3 = Color3.new(1, 1, 1)
  ...
  ib.MouseButton1Click:Connect(function()
    hr = not hr
    af(ae.Toggle, 0.35)
    a:Create(hz, TweenInfo.new(0.22, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Position = ((hr and UDim2.new(1, -21, 0.5, -9)) or UDim2.new(0, 3, 0.5, -9))}):Play()
    a:Create(hx, TweenInfo.new(0.22), {BackgroundColor3 = ((hr and ad.BloodBright) or Color3.fromRGB(45, 15, 20)), BackgroundTransparency = ((hr and 0.1) or 0.5)}):Play()
    if hq then
      hq(hr)
    end
  end)
end
function gt.CreateSlider(ic, id, ie, ig, ih, ii)
  local ij = (ih or ie)
  local ik = Instance.new("Frame")
  ik.Size = UDim2.new(1, 0, 0, 58)
  ik.BackgroundColor3 = Color3.fromRGB(22, 10, 14)
  ik.BackgroundTransparency = 0.4
  ik.BorderSizePixel = 0
  ik.Parent = ic
  local il = Instance.new("UICorner")
  il.CornerRadius = UDim.new(0, 8)
  il.Parent = ik
  local im = Instance.new("UIStroke")
  im.Color = ad.CardStroke
  im.Transparency = 0.6
  im.Thickness = 1
  im.Parent = ik
  local io = Instance.new("TextLabel")
  io.Size = UDim2.new(1, -80, 0, 24)
  io.Position = UDim2.new(0, 14, 0, 8)
  io.Text = id
  io.TextColor3 = ad.TextPrimary
  io.TextSize = 13
  io.Font = Enum.Font.GothamMedium
  io.TextXAlignment = Enum.TextXAlignment.Left
  io.BackgroundTransparency = 1
  io.Parent = ik
  local ip = Instance.new("Frame")
  ip.Size = UDim2.new(0, 48, 0, 20)
  ip.Position = UDim2.new(1, -62, 0, 8)
  ip.BackgroundColor3 = Color3.fromRGB(38, 14, 20)
  ip.BorderSizePixel = 0
  ip.Parent = ik
  local iq = Instance.new("UICorner")
  iq.CornerRadius = UDim.new(0, 6)
  iq.Parent = ip
  local ir = Instance.new("TextLabel")
  ir.Size = UDim2.new(1, 0, 1, 0)
  ir.Text = tostring(ij)
  ir.TextColor3 = ad.BloodGlow
  ir.TextSize = 12
  ir.Font = Enum.Font.GothamBold
  ir.BackgroundTransparency = 1
  ir.Parent = ip
  local is = Instance.new("Frame")
  is.Size = UDim2.new(1, -28, 0, 7)
  is.Position = UDim2.new(0, 14, 0, 38)
  is.BackgroundColor3 = Color3.fromRGB(40, 15, 22)
  is.BorderSizePixel = 0
  is.Parent = ik
  local it = Instance.new("UICorner")
  it.CornerRadius = UDim.new(1, 0)
  it.Parent = is
  local iu = math.clamp((((ij - ie)) / ((ig - ie))), 0, 1)
  local iv = Instance.new("Frame")
  iv.Size = UDim2.new(iu, 0, 1, 0)
  iv.BackgroundColor3 = ad.BloodBright
  iv.BorderSizePixel = 0
  iv.Parent = is
  local iw = Instance.new("UICorner")
  iw.CornerRadius = UDim.new(1, 0)
  iw.Parent = iv
  local ix = Instance.new("Frame")
  ix.Size = UDim2.new(0, 15, 0, 15)
  ix.AnchorPoint = Vector2.new(0.5, 0.5)
  ix.Position = UDim2.new(iu, 0, 0.5, 0)
  ix.BackgroundColor3 = Color3.new(1, 1, 1)
  ix.BorderSizePixel = 0
  ix.Parent = is
  local iy = Instance.new("UICorner")
  iy.CornerRadius = UDim.new(1, 0)
  iy.Parent = ix
  local iz = false
  local function ja(jb)
    local jc = math.clamp((jb.Position.X - is.AbsolutePosition.X), 0, is.AbsoluteSize.X)
    local jd = (jc / is.AbsoluteSize.X)
    ij = math.floor((ie + ((jd * ((ig - ie))))))
    ir.Text = tostring(ij)
    a:Create(iv, TweenInfo.new(0.04), {Size = UDim2.new(jd, 0, 1, 0)}):Play()
    a:Create(ix, TweenInfo.new(0.04), {Position = UDim2.new(jd, 0, 0.5, 0)}):Play()
    if ii then
      ii(ij)
    end
  end
  is.InputBegan:Connect(function(je)
    if ((je.UserInputType == Enum.UserInputType.MouseButton1) or (je.UserInputType == Enum.UserInputType.Touch)) then
      iz = true
      ja(je)
    end
  end)
  b.InputEnded:Connect(function(jf)
    if ((jf.UserInputType == Enum.UserInputType.MouseButton1) or (jf.UserInputType == Enum.UserInputType.Touch)) then
      iz = false
    end
  end)
  b.InputChanged:Connect(function(jg)
    if (iz and (((jg.UserInputType == Enum.UserInputType.MouseMovement) or (jg.UserInputType == Enum.UserInputType.Touch)))) then
      ja(jg)
    end
  end)
end
function gt.CreateDropdown(jh, ji, jj, jk, jl)
  local jm = (jk or jj[1])
  local jn = false
  local jo = Instance.new("Frame")
  jo.Size = UDim2.new(1, 0, 0, 44)
  jo.BackgroundColor3 = Color3.fromRGB(22, 10, 14)
  jo.BackgroundTransparency = 0.4
  jo.BorderSizePixel = 0
  jo.ClipsDescendants = true
  jo.Parent = jh
  local jp = Instance.new("UICorner")
  jp.CornerRadius = UDim.new(0, 8)
  jp.Parent = jo
  local jq = Instance.new("UIStroke")
  jq.Color = ad.CardStroke
  jq.Transparency = 0.6
  jq.Thickness = 1
  jq.Parent = jo
  local jr = Instance.new("Frame")
  jr.Size = UDim2.new(1, 0, 0, 44)
  jr.BackgroundTransparency = 1
  jr.Parent = jo
  local js = Instance.new("TextLabel")
  js.Size = UDim2.new(0.45, 0, 1, 0)
  js.Position = UDim2.new(0, 14, 0, 0)
  js.Text = ji
  js.TextColor3 = ad.TextPrimary
  js.TextSize = 13
  js.Font = Enum.Font.GothamMedium
  js.TextXAlignment = Enum.TextXAlignment.Left
  js.BackgroundTransparency = 1
  js.Parent = jr
  local jt = Instance.new("TextLabel")
  jt.Size = UDim2.new(0.55, -46, 1, 0)
  jt.Position = UDim2.new(0.45, 0, 0, 0)
  jt.Text = jm
  jt.TextColor3 = ad.BloodGlow
  jt.TextSize = 12
  jt.Font = Enum.Font.GothamSemibold
  jt.TextXAlignment = Enum.TextXAlignment.Right
  jt.BackgroundTransparency = 1
  jt.Parent = jr
  local ju = Instance.new("Frame")
  ju.Size = UDim2.new(0, 16, 0, 16)
  ju.Position = UDim2.new(1, -22, 0.5, 0)
  ju.AnchorPoint = Vector2.new(0.5, 0.5)
  ju.BackgroundTransparency = 1
  ju.Parent = jr
  local jv = Instance.new("Frame")
  jv.Size = UDim2.new(0, 6, 0, 2)
  jv.Position = UDim2.new(0.5, -2, 0.5, 0)
  jv.AnchorPoint = Vector2.new(0.5, 0.5)
  jv.Rotation = 45
  jv.BackgroundColor3 = ad.BloodBright
  jv.BorderSizePixel = 0
  jv.Parent = ju
  local jw = Instance.new("Frame")
  jw.Size = UDim2.new(0, 6, 0, 2)
  jw.Position = UDim2.new(0.5, 2, 0.5, 0)
  jw.AnchorPoint = Vector2.new(0.5, 0.5)
  jw.Rotation = -45
  jw.BackgroundColor3 = ad.BloodBright
  jw.BorderSizePixel = 0
  jw.Parent = ju
  local jx = Instance.new("UICorner")
  jx.CornerRadius = UDim.new(1, 0)
  jx.Parent = jv
  local jy = Instance.new("UICorner")
  jy.CornerRadius = UDim.new(1, 0)
  jy.Parent = jw
  local jz = Instance.new("TextButton")
  jz.Size = UDim2.new(1, 0, 1, 0)
  jz.BackgroundTransparency = 1
  jz.Text = ""
  jz.ZIndex = 2
  jz.Parent = jr
  jz.MouseEnter:Connect(function()
    a:Create(jq, TweenInfo.new(0.15), {Color = ad.BloodBright, Transparency = 0.35}):Play()
  end)
  jz.MouseLeave:Connect(function()
    a:Create(jq, TweenInfo.new(0.15), {Color = ad.CardStroke, Transparency = 0.6}):Play()
  end)
  local ka = Instance.new("ScrollingFrame")
  ka.Size = UDim2.new(1, -24, 0, math.min((#jj * 32), 130))
  ka.Position = UDim2.new(0, 12, 0, 48)
  ka.BackgroundTransparency = 1
  ka.BorderSizePixel = 0
  ka.ScrollBarThickness = 3
  ka.ScrollBarImageColor3 = ad.BloodBright
  ka.CanvasSize = UDim2.new(0, 0, 0, (#jj * 32))
  ka.Parent = jo
  local kb = Instance.new("UIListLayout")
  kb.Padding = UDim.new(0, 4)
  kb.Parent = ka
  for kc, kd in ipairs(jj) do
    local ke = Instance.new("TextButton")
    ke.Size = UDim2.new(1, 0, 0, 28)
    ke.BackgroundColor3 = Color3.fromRGB(32, 13, 18)
    ke.BackgroundTransparency = 0.4
    ke.Text = ("  " .. kd)
    ke.TextColor3 = ((((kd == jm)) and ad.BloodGlow) or ad.TextSecondary)
    ke.TextSize = 12
    ke.Font = Enum.Font.Gotham
    ke.TextXAlignment = Enum.TextXAlignment.Left
    ke.AutoButtonColor = false
    ke.Parent = ka
    local kf = Instance.new("UICorner")
    kf.CornerRadius = UDim.new(0, 6)
    kf.Parent = ke
    ke.MouseEnter:Connect(function()
      a:Create(ke, TweenInfo.new(0.12), {TextColor3 = ad.BloodGlow, BackgroundTransparency = 0.15}):Play()
    end)
    ke.MouseLeave:Connect(function()
      local kg = ((((kd == jm)) and ad.BloodGlow) or ad.TextSecondary)
      a:Create(ke, TweenInfo.new(0.12), {TextColor3 = kg, BackgroundTransparency = 0.4}):Play()
    end)
    ke.MouseButton1Click:Connect(function()
      af(ae.Click, 0.3)
      jm = kd
      jt.Text = jm
      jn = false
      a:Create(jo, TweenInfo.new(0.25, Enum.EasingStyle.Quart), {Size = UDim2.new(1, 0, 0, 44)}):Play()
      a:Create(ju, TweenInfo.new(0.2, Enum.EasingStyle.Quart), {Rotation = 0}):Play()
      if jl then
        jl(jm)
      end
    end)
  end
  jz.MouseButton1Click:Connect(function()
    jn = not jn
    af(ae.Click, 0.25)
    local kh = ((jn and ((54 + math.min((#jj * 32), 130)))) or 44)
    a:Create(jo, TweenInfo.new(0.3, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Size = UDim2.new(1, 0, 0, kh)}):Play()
    a:Create(ju, TweenInfo.new(0.25, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Rotation = ((jn and 180) or 0)}):Play()
  end)
end
function gt.CreateTextInput(ki, kj, kk, kl)
  local km = Instance.new("Frame")
  km.Size = UDim2.new(1, 0, 0, 44)
  km.BackgroundColor3 = Color3.fromRGB(22, 10, 14)
  km.BackgroundTransparency = 0.4
  km.BorderSizePixel = 0
  km.Parent = ki
  local kn = Instance.new("UICorner")
  kn.CornerRadius = UDim.new(0, 8)
  kn.Parent = km
  local ko = Instance.new("UIStroke")
  ko.Color = ad.CardStroke
  ko.Transparency = 0.6
  ko.Thickness = 1
  ko.Parent = km
  local kp = Instance.new("TextLabel")
  kp.Size = UDim2.new(0.4, 0, 1, 0)
  kp.Position = UDim2.new(0, 14, 0, 0)
  kp.Text = kj
  kp.TextColor3 = ad.TextPrimary
  kp.TextSize = 13
  kp.Font = Enum.Font.GothamMedium
  kp.TextXAlignment = Enum.TextXAlignment.Left
  kp.BackgroundTransparency = 1
  kp.Parent = km
  local kq = Instance.new("TextBox")
  kq.Size = UDim2.new(0.6, -24, 0, 28)
  kq.Position = UDim2.new(0.4, 10, 0.5, -14)
  kq.BackgroundColor3 = Color3.fromRGB(15, 6, 10)
  kq.BackgroundTransparency = 0.3
  kq.TextColor3 = ad.TextPrimary
  kq.PlaceholderColor3 = ad.TextMuted
  kq.PlaceholderText = (kk or "type…")
  kq.Text = ""
  kq.TextSize = 12
  kq.Font = Enum.Font.Gotham
  kq.ClearTextOnFocus = false
  kq.Parent = km
  local kr = Instance.new("UICorner")
  kr.CornerRadius = UDim.new(0, 6)
  kr.Parent = kq
  kq.Focused:Connect(function()
    a:Create(ko, TweenInfo.new(0.15), {Color = ad.BloodBright, Transparency = 0.3}):Play()
  end)
  kq.FocusLost:Connect(function(ks)
    a:Create(ko, TweenInfo.new(0.15), {Color = ad.CardStroke, Transparency = 0.6}):Play()
    if kl then
      kl(kq.Text, ks)
    end
  end)
end
function gt.CreateKeybind(kt, ku, kv, kw)
  local kx = (kv or Enum.KeyCode.RightControl)
  local ky = false
  local kz = Instance.new("Frame")
  kz.Size = UDim2.new(1, 0, 0, 44)
  kz.BackgroundColor3 = Color3.fromRGB(22, 10, 14)
  kz.BackgroundTransparency = 0.4
  kz.BorderSizePixel = 0
  kz.Parent = kt
  local la = Instance.new("UICorner")
  la.CornerRadius = UDim.new(0, 8)
  la.Parent = kz
  local lb = Instance.new("UIStroke")
  lb.Color = ad.CardStroke
  lb.Transparency = 0.6
  lb.Thickness = 1
  lb.Parent = kz
  local lc = Instance.new("TextLabel")
  lc.Size = UDim2.new(1, -120, 1, 0)
  lc.Position = UDim2.new(0, 14, 0, 0)
  lc.Text = ku
  lc.TextColor3 = ad.TextPrimary
  lc.TextSize = 13
  lc.Font = Enum.Font.GothamMedium
  lc.TextXAlignment = Enum.TextXAlignment.Left
  lc.BackgroundTransparency = 1
  lc.Parent = kz
  local ld = Instance.new("TextButton")
  ld.Size = UDim2.new(0, 96, 0, 26)
  ld.Position = UDim2.new(1, -108, 0.5, -13)
  ld.BackgroundColor3 = Color3.fromRGB(38, 15, 20)
  ld.BackgroundTransparency = 0.2
  ld.Text = kx.Name
  ld.TextColor3 = ad.BloodGlow
  ld.TextSize = 11
  ld.Font = Enum.Font.GothamBold
  ld.AutoButtonColor = false
  ld.Parent = kz
  local le = Instance.new("UICorner")
  le.CornerRadius = UDim.new(0, 6)
  le.Parent = ld
  local lf = Instance.new("UIStroke")
  lf.Color = ad.BloodBright
  lf.Transparency = 0.6
  lf.Thickness = 1
  lf.Parent = ld
  ...
  b.InputBegan:Connect(function(lg)
    if (ky and (lg.UserInputType == Enum.UserInputType.Keyboard)) then
      kx = lg.KeyCode
      ld.Text = kx.Name
      lf.Color = ad.BloodBright
      ky = false
...
local lk = 50
local ll = 2
local lm = "Stage 1 — 1 win"
local ln = false
local lo = nil
local function lp(lq)
  if lq then
    if not lo then
      lo = c.Stepped:Connect(function()
        pcall(function()
          if h.Character then
            for lr, ls in ipairs(h.Character:GetDescendants()) do
              if (ls:IsA("BasePart") and ls.CanCollide) then
                ls.CanCollide = false
              end
            end
          end
    ...
    if ((lo and not ln) and not lh) then
      lo:Disconnect()
      lo = nil
    end
  end
...
local function lt()
  local lu = {}
  local lv = workspace:FindFirstChild("Checkpoints")
  local lw = workspace:FindFirstChild("Stages")
  local lx = ((((lv and (#lv:GetChildren() > 0))) and lv) or (((((lw and (#lw:GetChildren() > 0))) and lw) or nil)))
  if lx then
    for ly, lz in ipairs(lx:GetChildren()) do
      local ma = tonumber(lz.Name:match("%d+"))
      local mb = (((lz:IsA("BasePart") and lz) or ((lz:IsA("Model") and ((lz.PrimaryPart or lz:FindFirstChildWhichIsA("BasePart")))))) or nil)
      if mb then
        table.insert(lu, {Index = (ma or 9999), Name = lz.Name, Part = mb, Position = (mb.Position + Vector3.new(0, 3.5, 0))})
      end
    end
  end
  table.sort(lu, function(mc, md)
    return (mc.Index < md.Index)
  end)
  return lu
end
local function me(mf)
  ...
  while lh do
    task.wait(0.1)
    local mh = h.Character
    local mi = (mh and mh:FindFirstChild("HumanoidRootPart"))
    if (mi and mg) then
      local mj = ((mi.Position - mg)).Magnitude
      if (mj > 60) then
        task.wait(0.6)
        return true
      end
...
local function ml(mm, mn)
  ...
  local mx = false
  local my = 0
  local mz = math.max(lk, 15)
  local na = Vector3.new(0, 0, -1)
  local function nb(nc)
    if (#mm == 1) then
      return mm[1], Vector3.new(0, 0, -1)
    end
    nc = math.clamp(nc, 0, mt)
    local nd = 0
    for ne = 1, #ms do
      local nf = ms[ne]
      if (nc <= (nd + nf)) then
        local ng = (((nf > 0) and (((nc - nd)) / nf)) or 0)
        return mm[ne]:Lerp(mm[(ne + 1)], ng), (mm[(ne + 1)] - mm[ne])
      end
      nd = (nd + nf)
    end
    return mm[#mm], (mm[#mm] - mm[(#mm - 1)])
  end
  local nr = c.RenderStepped:Connect(function(nh)
    if mw then
      return
    end
    pcall(function()
      local ni = h.Character
      local nj = (ni and ni:FindFirstChild("HumanoidRootPart"))
      local nk = (ni and ni:FindFirstChild("Humanoid"))
      if ((not nj or not nk) or (nk.Health <= 0)) then
        mw = true
        return
      end
      if (#mm == 0) then
      ...
      local nl = math.max(lk, 15)
      local nm = (900 * nh)
      mz = (mz + math.clamp((nl - mz), -nm, nm))
      my = (my + math.min((mz * math.min(nh, 0.1)), 35))
      if (my >= mt) then
        my = mt
        mx = true
      end
      local nn, no = nb(my)
      local np = (((no.Magnitude > 0.01) and no.Unit) or Vector3.new(0, 0, -1))
      nj.AssemblyLinearVelocity = Vector3.zero
      nj.AssemblyAngularVelocity = Vector3.zero
      local nq = Vector3.new(np.X, 0, np.Z)
      if (nq.Magnitude > 0.01) then
        na = nq.Unit
      end
      nj.CFrame = CFrame.lookAt(nn, (nn + na), Vector3.new(0, 1, 0))
    end)
  end)
  local ns = math.max(15, ((mt / mr) + 60))
  local nt = tick()
  while (((not mx and lh) and not mw) and ((tick() - nt) < ns)) do
    task.wait(0.03)
  end
  mw = true
  if nr then
  ...
  pcall(function()
    local nu = h.Character
    local nv = (nu and nu:FindFirstChild("Humanoid"))
    if nv then
      nv.PlatformStand = false
      nv.AutoRotate = true
      pcall(function()
        nv:ChangeState(Enum.HumanoidStateType.Running)
      end)
    end
  end)
  lp(false)
...
local function nw()
  ...
  task.spawn(function()
    local nx = 0
    for ny = 1, 16 do
      if (mk[ny] and (#mk[ny] > 0)) then
        nx = (nx + 1)
      end
    end
    if (nx > 0) then
      ca("Auto Win", string.format("%d stages baked. Speed: %d", nx, lk), 3.5, "Success")
    else
      ca("Auto Win", "No baked routes — using checkpoint scan", 3.5, "Warning")
    end
    while lh do
      local nz = h.Character
      local oa = (nz and nz:FindFirstChild("HumanoidRootPart"))
      local ob = (nz and nz:FindFirstChild("Humanoid"))
      if ((not oa or not ob) or (ob.Health <= 0)) then
        task.wait(0.5)
      else
        if (nx > 0) then
          local oc = tonumber(lm:match("%d+"))
          if ((oc and mk[oc]) and (#mk[oc] > 0)) then
            local od = mk[oc]
            ca(("Stage " .. oc), string.format("%d waypoints", #od), 1.8, "Info")
            local oe = ml(od, oc)
            if (oe and lh) then
              du = (du + ((dv[oc] or 1)))
              ca(("Stage " .. oc), "Complete!", 2, "Success")
              me(oe)
              task.wait(ll)
            else
              task.wait(0.5)
          ...
          if (#of > 0) then
            local og = {}
            for oh, oi in ipairs(of) do
              table.insert(og, oi.Position)
            end
            ml(og)
            du = (du + 1)
            task.wait(ll)
...
local function oj()
  ...
  pcall(function()
    if (h.Character and h.Character:FindFirstChild("Humanoid")) then
      local ok = h.Character.Humanoid
      ok.PlatformStand = false
      pcall(function()
        ok:ChangeState(Enum.HumanoidStateType.Running)
      end)
    end
  end)
end
local ol, om = false, 16
local on, oo = false, 50
local op, oq, os
local function ot(ou)
  if (((op == nil) and ou.WalkSpeed) and (ou.WalkSpeed > 0)) then
    op = ou.WalkSpeed
  end
  if (((oq == nil) and ou.JumpPower) and (ou.JumpPower > 0)) then
    oq = ou.JumpPower
  end
  if (((os == nil) and ou.JumpHeight) and (ou.JumpHeight > 0)) then
    os = ou.JumpHeight
  end
end
local function ov()
  pcall(function()
    local ow = h.Character
    local ox = (ow and ow:FindFirstChild("Humanoid"))
    if not ox then
      return
    end
    if ol then
    ...
    if on then
      ox.UseJumpPower = true
      ox.JumpPower = oo
      ox.JumpHeight = (((oo * oo)) / 392.4)
    elseif (oq ~= nil) then
      ox.JumpPower = oq
      ox.JumpHeight = (os or 7.2)
    end
  end)
end
local oy, oz = false, nil
  ...
  oz = task.spawn(function()
    while oy do
      if b:IsKeyDown(Enum.KeyCode.Space) then
        local pc = h.Character
        local pd = (pc and pc:FindFirstChildOfClass("Humanoid"))
        if (pd and (pd.Health > 0)) then
          pcall(function()
            pd:ChangeState("Jumping")
            task.wait(0.04)
            pd:ChangeState("Seated")
          end)
        end
      end
      task.wait(0.04)
...
local function pi(pj)
  pcall(function()
    local pk = h.Character
    local pl = (pk and pk:FindFirstChild("HumanoidRootPart"))
    if ((pl and pj) and pj.Parent) then
      pl.CFrame = pj.CFrame
    end
  end)
end
local function pm(pn)
  ...
  pg = pn.ChildAdded:Connect(function(po)
    if ((po.Name == "SpecialKey_Normal") or po.Name:find("SpecialKey")) then
      task.delay(0.15, function()
        pi(po)
        ca("Special Key", "Spawned — teleported", 2, "Success")
      end)
    end
  end)
end
local function pp(pq)
  ...
  pm(workspace:FindFirstChild("SpecialKeys"))
  ph = workspace.ChildAdded:Connect(function(pr)
    if (pr.Name == "SpecialKeys") then
      if pg then
        pg:Disconnect()
      end
      pm(pr)
  ...
  pf = task.spawn(function()
    while pe do
      pcall(function()
        local ps = h.Character
        local pt = (ps and ps:FindFirstChild("HumanoidRootPart"))
        local pu = workspace:FindFirstChild("SpecialKeys")
        if (pt and pu) then
          for pv, pw in ipairs(pu:GetChildren()) do
            local px = ((pw.Name == "SpecialKey_Normal") or pw.Name:find("SpecialKey"))
            if (px and (((pt.Position - pw.Position)).Magnitude > 6)) then
              pt.CFrame = pw.CFrame
              break
            end
          end
...
local function py()
  pcall(function()
    local pz = h.Character
    local qa = (pz and pz:FindFirstChild("HumanoidRootPart"))
    if not qa then
      ca("Special Key", "No character", 2, "Danger")
      return
    end
    local qb = workspace:FindFirstChild("SpecialKeys")
    local qc
    if qb then
      for qd, qe in ipairs(qb:GetChildren()) do
        if (qe.Name == "SpecialKey_Normal") then
          qc = qe
          break
        end
      end
      if not qc then
        for qf, qg in ipairs(qb:GetChildren()) do
          if qg.Name:find("SpecialKey") then
            qc = qg
            break
          end
        end
    ...
    if not qc then
      ca("Special Key", "None spawned yet", 2, "Danger")
      return
    end
    qa.CFrame = qc.CFrame
    ca("Special Key", "Teleported!", 2, "Success")
  end)
end
local function qh(qi)
  local qj = false
  pcall(function()
    local qk = {openurl, openUrl, OpenUrl, open_url, qh, windowopen, WindowOpen}
    for ql, qm in ipairs(qk) do
      if (type(qm) == "function") then
        local qn = pcall(qm, qi)
        if (qn and not qj) then
          qj = true
        end
      end
    end
  ...
  pcall(function()
    local qo = ((getgenv and getgenv()) or {})
    for qp, qq in ipairs({"openurl", "openUrl", "OpenUrl", "open_url", "OpenURL", "windowopen"}) do
      if (type(qo[qq]) == "function") then
        local qr = pcall(qo[qq], qi)
        if (qr and not qj) then
          qj = true
        end
      end
    end
...
local function qs()
  local qt = "https://discord.gg/NErP8zzq2"
  if qh(qt) then
    ca("Discord", "Opening invite…", 3, "Success")
  else
    pcall(function()
    ...
    ca("Discord", "Link copied to clipboard", 4, "Warning")
  end
end
...
local qu = ge("Dashboard", "◆")
local qv = gt.CreateSection(qu, "System Control")
local qw = gt.CreateSection(qu, "Win Counter")
local qx = Instance.new("TextLabel")
qx.Size = UDim2.new(1, 0, 0, 48)
qx.BackgroundColor3 = Color3.fromRGB(30, 8, 12)
qx.BackgroundTransparency = 0.35
qx.BorderSizePixel = 0
qx.Font = Enum.Font.GothamBold
qx.TextSize = 30
qx.TextColor3 = ad.BloodGlow
qx.Text = "0"
qx.LayoutOrder = 1
qx.Parent = qw
local qy = Instance.new("UICorner")
qy.CornerRadius = UDim.new(0, 8)
qy.Parent = qx
local qz = Instance.new("TextLabel")
qz.Size = UDim2.new(1, 0, 0, 18)
qz.Position = UDim2.new(0, 0, 0, 44)
qz.BackgroundTransparency = 1
qz.Font = Enum.Font.GothamMedium
qz.TextSize = 12
qz.TextColor3 = ad.TextSecondary
qz.Text = "Session Wins"
qz.LayoutOrder = 2
qz.Parent = qw
c.Heartbeat:Connect(function()
  local ra = du
  ...
  if (rb ~= "") then
    ra = (math.floor((ra * 10)) / 10)
    qx.Text = (((((((ra % 1) == 0)) and tostring(ra)) or string.format("%.1f", ra))) .. rb)
  else
    qx.Text = tostring(ra)
  end
end)
gt.CreateButton(qv, "Send Network Ping", "Broadcast a live latency check", function()
  ca("Network", ("Server responded in " .. (math.random(18, 42) .. "ms")), 3, "Success")
end)
gt.CreateSlider(qv, "Process Priority", 1, 100, 85, function()

end)
local rc = ge("Auto Win", "☠")
local rd = gt.CreateSection(rc, "Auto Win Engine")
gt.CreateToggle(rd, "Auto Win", "Glide through the course automatically", false, function(re)
  if re then
    nw()
  else
    oj()
...
gt.CreateSlider(rd, "Movement Speed (Studs/s)", 50, 300, 50, function(rf)
  lk = rf
end)
gt.CreateSlider(rd, "Loop Delay (s)", 1, 5, 2, function(rg)
  ll = rg
end)
gt.CreateDropdown(rd, "Target Stage", {"Stage 1 — 1 win", "Stage 2 — 3 wins", "Stage 3 — 10 wins", "Stage 4 — 20 wins", "Stage 5 — 50 wins", "Stage 6 — 100 wins", "Stage 7 — 150 wins", "Stage 8 — 300 wins", "Stage 9 — 500 wins", "Stage 10 — 1k wins", "Stage 11 — 2.5k wins", "Stage 12 — 10k wins", "Stage 13 — 50k wins", "Stage 14 — 50k wins", "Stage 15 — 150k wins", "Stage 16 — 350k wins"}, "Stage 1 — 1 win", function(rh)
  lm = rh
  ca("Target Stage", rh, 2, "Info")
end)
gt.CreateToggle(rd, "Safety NoClip", "Disable collision on character", true, function(ri)
  ln = ri
  lp(ri)
end)
local rj = gt.CreateSection(rc, "Special Keys")
gt.CreateToggle(rj, "Auto TP to Special Key", "Teleport the moment one spawns", false, function(rk)
  pp(rk)
  ca("Special Keys", ("Auto TP: " .. (((rk and "ON") or "OFF"))), 2, "Info")
end)
gt.CreateButton(rj, "TP to Special Key", "Manual teleport", function()
  py()
end)
local rl = ge("Player", "⚔")
local rm = gt.CreateSection(rl, "Locomotion")
gt.CreateToggle(rm, "WalkSpeed Boost", "Toggle walkspeed on/off", false, function(rn)
  ol = rn
  ov()
end)
gt.CreateSlider(rm, "WalkSpeed Value", 16, 1000, 16, function(ro)
  om = ro
  if ol then
    ov()
  end
end)
gt.CreateToggle(rm, "JumpPower Boost", "Toggle jump power on/off", false, function(rp)
  on = rp
  ov()
end)
gt.CreateSlider(rm, "JumpPower Value", 50, 1000, 50, function(rq)
  oo = rq
  if on then
    ov()
  end
end)
gt.CreateToggle(rm, "Infinite Jump", "Hold Space to auto-jump", false, function(rr)
  pa(rr)
end)
local rs = ge("Settings", "⚙")
local rt = gt.CreateSection(rs, "Preferences")
gt.CreateKeybind(rt, "Toggle Menu Keybind", Enum.KeyCode.RightControl, function(ru)
  fd = ru
  ca("Keybind", ("New key: " .. ru.Name), 3, "Success")
end)
gt.CreateButton(rt, "Unload UI", "Destroy all instances and restore lighting", function()
  ca("Unloading", "GORE LAB shutting down…", 1.2, "Danger")
  task.wait(0.6)
  if aj then
    aj:Destroy()
  end
...
local rv = gt.CreateSection(rs, "Atmosphere")
gt.CreateToggle(rv, "Frosted Depth Blur", "Blur the 3D scene behind the UI", false, function(rw)
  ak = rw
  if aj then
    aj.Enabled = (rw and cw.Visible)
  end
end)
gt.CreateSlider(rv, "Window Transparency", 5, 80, 8, function(rx)
  cw.BackgroundTransparency = (rx / 100)
end)
gt.CreateSlider(rv, "Sheen Intensity", 0, 100, 70, function(ry)
  de.BackgroundTransparency = (((100 - ry)) / 100)
end)
local rz = ge("Credits", "★")
local sa = gt.CreateSection(rz, "GORE LAB")
gt.CreateButton(sa, "Created by lostpostcard", "Roblox User ID: 11594416001", function()
  ca("Creator", "Opening Roblox profile…", 3, "Info")
  pcall(function()
    game:GetService("GuiService"):OpenBrowserUrl("https://www.roblox.com/users/11594416001/profile")
  end)
end)
gt.CreateButton(sa, "Discord Server", "discord.gg/NErP8zzq2", function()
  qs()
end)
gt.CreateButton(sa, "Unload UI", "Close everything cleanly", function()
  task.wait(0.3)
  if aj then
    aj:Destroy()
  end
...
fw["Dashboard"].Select()
by.ZIndex = 800
local sb = "gorefirstkeyreleased"
local sc = false
_G.GORELocked = true
cw.Visible = false
local sd = i:FindFirstChild("GORELock")
if sd then
  sd:Destroy()
end
local se = Instance.new("ScreenGui")
se.Name = "GORELock"
se.ResetOnSpawn = false
se.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
se.DisplayOrder = 999
se.IgnoreGuiInset = true
se.Parent = i
local sf = Instance.new("Frame")
sf.Size = UDim2.new(0, 380, 0, 400)
sf.AnchorPoint = Vector2.new(0.5, 0.5)
sf.Position = UDim2.new(0.5, 0, 0.5, 0)
sf.BackgroundColor3 = ad.WindowGlass
sf.BackgroundTransparency = 0.05
sf.BorderSizePixel = 0
sf.ClipsDescendants = true
sf.Active = true
sf.Parent = se
local sg = Instance.new("UICorner")
sg.CornerRadius = UDim.new(0, 16)
sg.Parent = sf
local sh = Instance.new("UIStroke")
sh.Color = ad.BloodBright
sh.Thickness = 1.8
sh.Parent = sf
sf.Size = UDim2.new(0, 0, 0, 0)
sf.BackgroundTransparency = 1
task.spawn(function()
  a:Create(sf, TweenInfo.new(0.5, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Size = UDim2.new(0, 380, 0, 400), BackgroundTransparency = 0.05}):Play()
end)
local si = false
local sj, sk, sl
sf.InputBegan:Connect(function(sm)
  if sc then
    return
  end
  if ((sm.UserInputType == Enum.UserInputType.MouseButton1) or (sm.UserInputType == Enum.UserInputType.Touch)) then
    si = true
    sj = sm.Position
    sk = sf.Position
    sm.Changed:Connect(function()
      if (sm.UserInputState == Enum.UserInputState.End) then
        si = false
      end
    end)
  end
end)
sf.InputChanged:Connect(function(sn)
  if ((sn.UserInputType == Enum.UserInputType.MouseMovement) or (sn.UserInputType == Enum.UserInputType.Touch)) then
    sl = sn
  end
end)
b.InputChanged:Connect(function(so)
  if ((so == sl) and si) then
    local sp = (so.Position - sj)
    local sq = UDim2.new(sk.X.Scale, (sk.X.Offset + sp.X), sk.Y.Scale, (sk.Y.Offset + sp.Y))
    a:Create(sf, TweenInfo.new(0.08, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Position = sq}):Play()
  end
end)
task.spawn(function()
  while ((sf and sf.Parent) and not sc) do
    a:Create(sh, TweenInfo.new(0.9, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {Transparency = 0.0, Thickness = 3}):Play()
    task.wait(0.9)
    a:Create(sh, TweenInfo.new(0.9, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {Transparency = 0.55, Thickness = 1.8}):Play()
    task.wait(0.9)
  end
end)
local sr = Instance.new("Frame")
sr.Size = UDim2.fromScale(1, 1)
sr.Position = UDim2.fromScale(0, 0)
sr.BackgroundColor3 = Color3.fromRGB(6, 2, 3)
sr.BackgroundTransparency = 0.15
sr.BorderSizePixel = 0
sr.ZIndex = 0
sr.Parent = sf
local ss = Instance.new("UIGradient")
ss.Color = ColorSequence.new({ColorSequenceKeypoint.new(0, Color3.fromRGB(35, 3, 8)), ColorSequenceKeypoint.new(0.5, Color3.fromRGB(6, 2, 3)), ColorSequenceKeypoint.new(1, Color3.fromRGB(50, 4, 10))})
ss.Rotation = 25
ss.Parent = sr
local st = Random.new(4421)
for su = 1, 26 do
  local sv = Instance.new("Frame")
  local sw = st:NextInteger(3, 14)
  sv.Size = UDim2.fromOffset(sw, math.max(3, math.floor((sw * st:NextNumber(0.45, 1.5)))))
  sv.Position = UDim2.fromScale(st:NextNumber(0.02, 0.98), st:NextNumber(0.03, 0.97))
  sv.Rotation = st:NextInteger(0, 360)
  sv.BackgroundColor3 = Color3.fromRGB(st:NextInteger(100, 220), 5, 14)
  sv.BackgroundTransparency = st:NextNumber(0.05, 0.35)
  sv.BorderSizePixel = 0
  sv.ZIndex = 0
  sv.Parent = sr
  local sx = Instance.new("UICorner")
  sx.CornerRadius = UDim.new(0.5, 0)
  sx.Parent = sv
end
task.spawn(function()
  while ((sf and sf.Parent) and not sc) do
    local sy = Instance.new("Frame")
    sy.Size = UDim2.fromOffset(st:NextInteger(3, 6), st:NextInteger(6, 12))
    sy.Position = UDim2.new(st:NextNumber(0.02, 0.98), 0, 0, -20)
    sy.BackgroundColor3 = Color3.fromRGB(st:NextInteger(160, 230), 5, 15)
    sy.BackgroundTransparency = 0.15
    sy.BorderSizePixel = 0
    sy.ZIndex = 1
    sy.Parent = sr
    local sz = Instance.new("UICorner")
    sz.CornerRadius = UDim.new(0.5, 0)
    sz.Parent = sy
    a:Create(sy, TweenInfo.new(st:NextNumber(2, 4), Enum.EasingStyle.Linear), {Position = UDim2.new(sy.Position.X.Scale, 0, 1, 20)}):Play()
    task.delay(4.2, function()
      if sy then
        sy:Destroy()
      end
    end)
    task.wait(st:NextNumber(0.8, 2.4))
  end
end)
local ta = Instance.new("Frame")
ta.Size = UDim2.new(1, 0, 0, 90)
ta.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
ta.BorderSizePixel = 0
ta.ZIndex = 1
ta.Parent = sf
local tb = Instance.new("UIGradient")
tb.Transparency = NumberSequence.new({NumberSequenceKeypoint.new(0, 0.94), NumberSequenceKeypoint.new(0.4, 0.98), NumberSequenceKeypoint.new(1, 1)})
tb.Rotation = 90
tb.Parent = ta
local tc = Instance.new("Frame")
tc.Size = UDim2.new(0, 60, 0, 3)
tc.AnchorPoint = Vector2.new(0.5, 0)
tc.Position = UDim2.new(0.5, 0, 0, 0)
tc.BackgroundColor3 = ad.BloodBright
tc.BorderSizePixel = 0
tc.ZIndex = 2
tc.Parent = sf
local td = Instance.new("UICorner")
td.CornerRadius = UDim.new(1, 0)
td.Parent = tc
local te = Instance.new("TextLabel")
te.Size = UDim2.new(0, 44, 0, 44)
te.AnchorPoint = Vector2.new(0.5, 0)
te.Position = UDim2.new(0.5, 0, 0, 22)
te.BackgroundColor3 = ad.CardGlass
...
te.ZIndex = 2
te.Parent = sf
local tf = Instance.new("UICorner")
tf.CornerRadius = UDim.new(0, 12)
tf.Parent = te
local tg = Instance.new("UIStroke")
tg.Color = ad.BloodBright
tg.Transparency = 0.35
tg.Thickness = 1.3
tg.Parent = te
task.spawn(function()
  while ((te and te.Parent) and not sc) do
    a:Create(tg, TweenInfo.new(0.7, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {Transparency = 0.05, Thickness = 2}):Play()
    task.wait(0.7)
    a:Create(tg, TweenInfo.new(0.7, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {Transparency = 0.55, Thickness = 1.3}):Play()
    task.wait(0.7)
  end
end)
local th = Instance.new("TextLabel")
th.Size = UDim2.new(1, 0, 0, 30)
th.Position = UDim2.new(0, 0, 0, 76)
th.BackgroundTransparency = 1
th.Font = Enum.Font.GothamBlack
th.TextSize = 24
th.TextColor3 = ad.TextPrimary
th.RichText = true
th.Text = "<b>GORE</b> <font color=\"rgb(220,18,30)\">LAB</font>"
th.TextXAlignment = Enum.TextXAlignment.Center
th.ZIndex = 2
th.Parent = sf
local ti = Instance.new("TextLabel")
ti.Size = UDim2.new(1, -40, 0, 16)
ti.AnchorPoint = Vector2.new(0.5, 0)
ti.Position = UDim2.new(0.5, 0, 0, 110)
ti.BackgroundTransparency = 1
ti.Font = Enum.Font.Gotham
ti.TextSize = 12
ti.TextColor3 = ad.TextSecondary
ti.Text = "Enter your access key to continue"
ti.TextXAlignment = Enum.TextXAlignment.Center
ti.ZIndex = 2
ti.Parent = sf
local tj = Instance.new("TextLabel")
tj.Size = UDim2.new(1, -48, 0, 14)
tj.AnchorPoint = Vector2.new(0.5, 0)
tj.Position = UDim2.new(0.5, 0, 0, 145)
tj.BackgroundTransparency = 1
tj.Font = Enum.Font.GothamBold
tj.TextSize = 10
tj.TextColor3 = ad.TextMuted
tj.TextXAlignment = Enum.TextXAlignment.Left
tj.Text = "ACCESS KEY"
tj.ZIndex = 2
tj.Parent = sf
local tk = Instance.new("TextBox")
tk.Size = UDim2.new(1, -48, 0, 42)
tk.AnchorPoint = Vector2.new(0.5, 0)
tk.Position = UDim2.new(0.5, 0, 0, 163)
tk.BackgroundColor3 = Color3.fromRGB(15, 6, 10)
tk.BackgroundTransparency = 0.2
tk.BorderSizePixel = 0
tk.Font = Enum.Font.GothamBold
tk.TextSize = 14
tk.TextColor3 = ad.TextPrimary
tk.PlaceholderText = "enter key…"
tk.PlaceholderColor3 = ad.TextMuted
tk.TextXAlignment = Enum.TextXAlignment.Center
tk.ClearTextOnFocus = false
tk.Text = ""
tk.ZIndex = 2
tk.Parent = sf
local tl = Instance.new("UICorner")
tl.CornerRadius = UDim.new(0, 8)
tl.Parent = tk
local tm = Instance.new("UIStroke")
tm.Color = ad.CardStroke
tm.Transparency = 0.5
tm.Thickness = 1
tm.Parent = tk
tk.Focused:Connect(function()
  a:Create(tm, TweenInfo.new(0.2), {Color = ad.BloodBright, Transparency = 0.2, Thickness = 1.4}):Play()
end)
tk.FocusLost:Connect(function()
  a:Create(tm, TweenInfo.new(0.2), {Color = ad.CardStroke, Transparency = 0.5, Thickness = 1}):Play()
end)
local tn = Instance.new("TextLabel")
tn.Size = UDim2.new(1, -48, 0, 16)
tn.AnchorPoint = Vector2.new(0.5, 0)
tn.Position = UDim2.new(0.5, 0, 0, 212)
tn.BackgroundTransparency = 1
tn.Font = Enum.Font.GothamMedium
tn.TextSize = 11
tn.TextColor3 = ad.TextSecondary
tn.TextXAlignment = Enum.TextXAlignment.Center
tn.Text = "Awaiting key…"
tn.ZIndex = 2
tn.Parent = sf
local to = Instance.new("TextButton")
to.Size = UDim2.new(1, -48, 0, 44)
to.AnchorPoint = Vector2.new(0.5, 0)
to.Position = UDim2.new(0.5, 0, 0, 240)
to.BackgroundColor3 = ad.BloodBright
to.BorderSizePixel = 0
to.AutoButtonColor = false
to.Font = Enum.Font.GothamBold
to.TextSize = 14
to.TextColor3 = Color3.fromRGB(255, 240, 240)
to.Text = "UNLOCK"
to.ZIndex = 2
to.Parent = sf
local tp = Instance.new("UICorner")
tp.CornerRadius = UDim.new(0, 10)
tp.Parent = to
local tq = Instance.new("UIStroke")
tq.Color = ad.BloodGlow
tq.Transparency = 0.4
tq.Thickness = 1.4
tq.Parent = to
to.MouseEnter:Connect(function()
  a:Create(to, TweenInfo.new(0.15), {BackgroundColor3 = ad.BloodGlow}):Play()
end)
to.MouseLeave:Connect(function()
  a:Create(to, TweenInfo.new(0.15), {BackgroundColor3 = ad.BloodBright}):Play()
end)
local tr = Instance.new("TextButton")
tr.Size = UDim2.new(1, -48, 0, 36)
tr.AnchorPoint = Vector2.new(0.5, 0)
tr.Position = UDim2.new(0.5, 0, 0, 296)
tr.BackgroundColor3 = Color3.fromRGB(24, 10, 14)
tr.BackgroundTransparency = 0.15
tr.BorderSizePixel = 0
tr.AutoButtonColor = false
tr.Font = Enum.Font.GothamBold
tr.TextSize = 12
tr.TextColor3 = ad.TextPrimary
tr.Text = "🔑  GET KEY"
tr.ZIndex = 2
tr.Parent = sf
local ts = Instance.new("UICorner")
ts.CornerRadius = UDim.new(0, 10)
ts.Parent = tr
local tt = Instance.new("UIStroke")
tt.Color = ad.BloodBright
tt.Transparency = 0.55
tt.Thickness = 1.2
tt.Parent = tr
tr.MouseEnter:Connect(function()
  a:Create(tr, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(38, 14, 20), BackgroundTransparency = 0}):Play()
  a:Create(tt, TweenInfo.new(0.15), {Color = ad.BloodGlow, Transparency = 0.25}):Play()
end)
tr.MouseLeave:Connect(function()
  a:Create(tr, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(24, 10, 14), BackgroundTransparency = 0.15}):Play()
  a:Create(tt, TweenInfo.new(0.15), {Color = ad.BloodBright, Transparency = 0.55}):Play()
end)
tr.MouseButton1Click:Connect(function()
  af(ae.Click, 0.4)
  local tu = "https://discord.gg/NErP8zzq2"
  local tv = pcall(function()
    if setclipboard then
      setclipboard(tu)
    end
  end)
  if tv then
    tn.Text = "Discord link copied to clipboard!"
    tn.TextColor3 = ad.Success
    tr.Text = "✅  COPIED"
    task.wait(1.5)
    ...
    tn.Text = "Couldn't copy — link: discord.gg/NErP8zzq2"
    tn.TextColor3 = ad.Warning
  end
end)
local tw = Instance.new("TextLabel")
tw.Size = UDim2.new(1, 0, 0, 18)
tw.AnchorPoint = Vector2.new(0.5, 1)
tw.Position = UDim2.new(0.5, 0, 1, -12)
tw.BackgroundTransparency = 1
tw.Font = Enum.Font.GothamMedium
tw.TextSize = 10
tw.TextColor3 = ad.TextMuted
tw.Text = "GORE LAB • OFFLINE GATE"
tw.TextXAlignment = Enum.TextXAlignment.Center
tw.ZIndex = 2
tw.Parent = sf
local tx = false
local function ty()
  ...
  local tz = tk.Text:gsub("%s+", "")
  if (#tz == 0) then
    tn.Text = "Enter a key first"
    tn.TextColor3 = ad.Warning
    af(ae.Notif, 0.3)
    return
  end
  ...
  task.spawn(function()
    ...
    if (tz:lower() == sb) then
      sc = true
      tn.Text = "Access granted"
      tn.TextColor3 = ad.Success
      af(ae.Toggle, 0.5)
      for ua, ub in ipairs(se:GetDescendants()) do
        if ub:IsA("GuiObject") then
          a:Create(ub, TweenInfo.new(0.35), {BackgroundTransparency = 1}):Play()
        end
        if ((ub:IsA("TextLabel") or ub:IsA("TextButton")) or ub:IsA("TextBox")) then
          a:Create(ub, TweenInfo.new(0.3), {TextTransparency = 1}):Play()
        end
        if ub:IsA("UIStroke") then
          a:Create(ub, TweenInfo.new(0.3), {Transparency = 1}):Play()
        end
      end
      task.wait(0.4)
      se:Destroy()
      ...
      ca("Welcome", ("Access granted — " .. h.DisplayName), 4, "Success")
    else
      tx = false
      to.Text = "UNLOCK"
      to.Active = true
      tn.Text = "Invalid key"
      tn.TextColor3 = ad.Danger
      af(ae.Notif, 0.5)
    end
  end)
end
...
tk.FocusLost:Connect(function(uc)
  if uc then
    ty()
  end
end)
