...
local ox, oy = false, 16
local oz, pa = false, 50
local pb, pc, pd
local function pe(pf)
  if (((pb == nil) and pf.WalkSpeed) and (pf.WalkSpeed > 0)) then
    pb = pf.WalkSpeed
  end
  if (((pc == nil) and pf.JumpPower) and (pf.JumpPower > 0)) then
    pc = pf.JumpPower
  end
  if (((pd == nil) and pf.JumpHeight) and (pf.JumpHeight > 0)) then
    pd = pf.JumpHeight
  end
end
local function pg()
  pcall(function()
    local ph = h.Character
    local pi = (ph and ph:FindFirstChild("Humanoid"))
    if not pi then
      return
    end
    if ox then
    ...
    if oz then
      pi.UseJumpPower = true
      pi.JumpPower = pa
      pi.JumpHeight = (((pa * pa)) / 392.4)
    elseif (pc ~= nil) then
      pi.JumpPower = pc
      pi.JumpHeight = (pd or 7.2)
    end
  end)
end
local pj, pk = false, nil
local function pl(pm)
  ...
  pk = task.spawn(function()
    while pj do
      if b:IsKeyDown(Enum.KeyCode.Space) then
        local pn = h.Character
        local po = (pn and pn:FindFirstChildOfClass("Humanoid"))
        if (po and (po.Health > 0)) then
          pcall(function()
            po:ChangeState("Jumping")
            task.wait(0.04)
            po:ChangeState("Seated")
          end)
        end
      end
      task.wait(0.04)
...
local function pt(pu)
  pcall(function()
    local pv = h.Character
    local pw = (pv and pv:FindFirstChild("HumanoidRootPart"))
    if ((pw and pu) and pu.Parent) then
      pw.CFrame = pu.CFrame
    end
  end)
end
local function px(py)
  ...
  pr = py.ChildAdded:Connect(function(pz)
    if ((pz.Name == "SpecialKey_Normal") or pz.Name:find("SpecialKey")) then
...
local function qa(qb)
  ...
  px(workspace:FindFirstChild("SpecialKeys"))
  ps = workspace.ChildAdded:Connect(function(qc)
    if (qc.Name == "SpecialKeys") then
      if pr then
        pr:Disconnect()
      end
      px(qc)
  ...
  pq = task.spawn(function()
    while pp do
      pcall(function()
        local qd = h.Character
        local qe = (qd and qd:FindFirstChild("HumanoidRootPart"))
        local qf = workspace:FindFirstChild("SpecialKeys")
        if (qe and qf) then
          for qg, qh in ipairs(qf:GetChildren()) do
            local qi = ((qh.Name == "SpecialKey_Normal") or qh.Name:find("SpecialKey"))
            if (qi and (((qe.Position - qh.Position)).Magnitude > 6)) then
              qe.CFrame = qh.CFrame
              break
            end
          end
...
local function qj()
  pcall(function()
    local qk = h.Character
    local ql = (qk and qk:FindFirstChild("HumanoidRootPart"))
    if not ql then
      cl("Special Key", "No character", 2, "Danger")
      return
    end
    local qm = workspace:FindFirstChild("SpecialKeys")
    local qn
    if qm then
      for qo, qp in ipairs(qm:GetChildren()) do
        if (qp.Name == "SpecialKey_Normal") then
          qn = qp
          break
        end
      end
      if not qn then
        for qq, qr in ipairs(qm:GetChildren()) do
          if qr.Name:find("SpecialKey") then
            qn = qr
            break
          end
        end
...
local function qs(qt)
  local qu = false
  pcall(function()
    local qv = {openurl, openUrl, OpenUrl, open_url, qs, windowopen, WindowOpen}
    for qw, qx in ipairs(qv) do
      if (type(qx) == "function") then
        local qy = pcall(qx, qt)
        if (qy and not qu) then
          qu = true
        end
      end
    end
  ...
  pcall(function()
    local qz = ((getgenv and getgenv()) or {})
    for ra, rb in ipairs({"openurl", "openUrl", "OpenUrl", "open_url", "OpenURL", "windowopen"}) do
      if (type(qz[rb]) == "function") then
        local rc = pcall(qz[rb], qt)
        if (rc and not qu) then
          qu = true
        end
      end
    end
...
local function rd()
  local re = "https://discord.gg/NErP8zzq2"
  if qs(re) then
...
local rf = gp("Dashboard", "◆")
local rg = he.CreateSection(rf, "System Control")
local rh = he.CreateSection(rf, "Win Counter")
local ri = Instance.new("TextLabel")
ri.Size = UDim2.new(1, 0, 0, 48)
ri.BackgroundColor3 = Color3.fromRGB(30, 8, 12)
ri.BackgroundTransparency = 0.35
ri.BorderSizePixel = 0
ri.Font = Enum.Font.GothamBold
ri.TextSize = 30
ri.TextColor3 = ao.BloodGlow
ri.Text = "0"
ri.LayoutOrder = 1
ri.Parent = rh
local rj = Instance.new("UICorner")
rj.CornerRadius = UDim.new(0, 8)
rj.Parent = ri
local rk = Instance.new("TextLabel")
rk.Size = UDim2.new(1, 0, 0, 18)
rk.Position = UDim2.new(0, 0, 0, 44)
rk.BackgroundTransparency = 1
rk.Font = Enum.Font.GothamMedium
rk.TextSize = 12
rk.TextColor3 = ao.TextSecondary
rk.Text = "Session Wins"
rk.LayoutOrder = 2
rk.Parent = rh
c.Heartbeat:Connect(function()
  local rl = ef
  ...
  if (rm ~= "") then
    ...
    ri.Text = (((((((rl % 1) == 0)) and tostring(rl)) or string.format("%.1f", rl))) .. rm)
  else
    ri.Text = tostring(rl)
  end
end)
he.CreateButton(rg, "Send Network Ping", "Broadcast a live latency check", function()
  cl("Network", ("Server responded in " .. (math.random(18, 42) .. "ms")), 3, "Success")
end)
he.CreateSlider(rg, "Process Priority", 1, 100, 85, function()

end)
local rn = gp("Auto Win", "☠")
local ro = he.CreateSection(rn, "Auto Win Engine")
he.CreateToggle(ro, "Auto Win", "Glide through the course automatically", false, function(rp)
  if rp then
    oh()
  else
    ov()
...
he.CreateSlider(ro, "Movement Speed (Studs/s)", 50, 300, 50, function(rq)
  lv = rq
end)
he.CreateSlider(ro, "Loop Delay (s)", 1, 5, 2, function(rr)
  lw = rr
end)
he.CreateDropdown(ro, "Target Stage", {"Stage 1 — 1 win", "Stage 2 — 3 wins", "Stage 3 — 10 wins", "Stage 4 — 20 wins", "Stage 5 — 50 wins", "Stage 6 — 100 wins", "Stage 7 — 150 wins", "Stage 8 — 300 wins", "Stage 9 — 500 wins", "Stage 10 — 1k wins", "Stage 11 — 2.5k wins", "Stage 12 — 10k wins", "Stage 13 — 50k wins", "Stage 14 — 50k wins", "Stage 15 — 150k wins", "Stage 16 — 350k wins"}, "Stage 1 — 1 win", function(rs)
  lx = rs
  cl("Target Stage", rs, 2, "Info")
end)
he.CreateToggle(ro, "Safety NoClip", "Disable collision on character", true, function(rt)
  ly = rt
  ma(rt)
end)
local ru = he.CreateSection(rn, "Special Keys")
he.CreateToggle(ru, "Auto TP to Special Key", "Teleport the moment one spawns", false, function(rv)
  qa(rv)
  cl("Special Keys", ("Auto TP: " .. (((rv and "ON") or "OFF"))), 2, "Info")
end)
he.CreateButton(ru, "TP to Special Key", "Manual teleport", function()
  qj()
end)
local rw = gp("Player", "⚔")
local rx = he.CreateSection(rw, "Locomotion")
he.CreateToggle(rx, "WalkSpeed Boost", "Toggle walkspeed on/off", false, function(ry)
  ox = ry
  pg()
end)
he.CreateSlider(rx, "WalkSpeed Value", 16, 1000, 16, function(rz)
  oy = rz
  if ox then
    pg()
  end
end)
he.CreateToggle(rx, "JumpPower Boost", "Toggle jump power on/off", false, function(sa)
  oz = sa
  pg()
end)
he.CreateSlider(rx, "JumpPower Value", 50, 1000, 50, function(sb)
  pa = sb
  if oz then
    pg()
  end
end)
he.CreateToggle(rx, "Infinite Jump", "Hold Space to auto-jump", false, function(sc)
  pl(sc)
end)
local sd = gp("Settings", "⚙")
local se = he.CreateSection(sd, "Preferences")
he.CreateKeybind(se, "Toggle Menu Keybind", Enum.KeyCode.RightControl, function(sf)
  fo = sf
  cl("Keybind", ("New key: " .. sf.Name), 3, "Success")
end)
he.CreateButton(se, "Unload UI", "Destroy all instances and restore lighting", function()
...
local sg = he.CreateSection(sd, "Atmosphere")
he.CreateToggle(sg, "Frosted Depth Blur", "Blur the 3D scene behind the UI", false, function(sh)
  av = sh
  if au then
    au.Enabled = (sh and dh.Visible)
  end
end)
he.CreateSlider(sg, "Window Transparency", 5, 80, 8, function(si)
  dh.BackgroundTransparency = (si / 100)
end)
he.CreateSlider(sg, "Sheen Intensity", 0, 100, 70, function(sj)
  dq.BackgroundTransparency = (((100 - sj)) / 100)
end)
local sk = gp("Credits", "★")
local sl = he.CreateSection(sk, "GORE LAB")
he.CreateButton(sl, "Created by lostpostcard", "Roblox User ID: 11594416001", function()
...
he.CreateButton(sl, "Discord Server", "discord.gg/NErP8zzq2", function()
  rd()
end)
he.CreateButton(sl, "Unload UI", "Close everything cleanly", function()
  task.wait(0.3)
  if au then
    au:Destroy()
  end
...
gh["Dashboard"].Select()
cj.ZIndex = 800
local sm = "gorefirstkeyreleased"
local sn = false
_G.GORELocked = true
dh.Visible = false
local so = i:FindFirstChild("GORELock")
if so then
  so:Destroy()
end
local sp = Instance.new("ScreenGui")
sp.Name = "GORELock"
sp.ResetOnSpawn = false
sp.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
sp.DisplayOrder = 999
sp.IgnoreGuiInset = true
sp.Parent = i
local sq = Instance.new("Frame")
sq.Size = UDim2.new(0, 380, 0, 400)
sq.AnchorPoint = Vector2.new(0.5, 0.5)
sq.Position = UDim2.new(0.5, 0, 0.5, 0)
sq.BackgroundColor3 = ao.WindowGlass
...
sr.CornerRadius = UDim.new(0, 16)
sr.Parent = sq
local ss = Instance.new("UIStroke")
ss.Color = ao.BloodBright
ss.Thickness = 1.8
ss.Parent = sq
sq.Size = UDim2.new(0, 0, 0, 0)
sq.BackgroundTransparency = 1
task.spawn(function()
  a:Create(sq, TweenInfo.new(0.5, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Size = UDim2.new(0, 380, 0, 400), BackgroundTransparency = 0.05}):Play()
end)
local st = false
local su, sv, sw
sq.InputBegan:Connect(function(sx)
  if sn then
    return
  end
  if ((sx.UserInputType == Enum.UserInputType.MouseButton1) or (sx.UserInputType == Enum.UserInputType.Touch)) then
    st = true
    su = sx.Position
    sv = sq.Position
    sx.Changed:Connect(function()
      if (sx.UserInputState == Enum.UserInputState.End) then
        st = false
      end
    end)
  end
end)
sq.InputChanged:Connect(function(sy)
  if ((sy.UserInputType == Enum.UserInputType.MouseMovement) or (sy.UserInputType == Enum.UserInputType.Touch)) then
    sw = sy
  end
end)
b.InputChanged:Connect(function(sz)
  if ((sz == sw) and st) then
    local ta = (sz.Position - su)
    local tb = UDim2.new(sv.X.Scale, (sv.X.Offset + ta.X), sv.Y.Scale, (sv.Y.Offset + ta.Y))
    a:Create(sq, TweenInfo.new(0.08, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Position = tb}):Play()
  end
end)
task.spawn(function()
  while ((sq and sq.Parent) and not sn) do
    a:Create(ss, TweenInfo.new(0.9, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {Transparency = 0.0, Thickness = 3}):Play()
    task.wait(0.9)
    a:Create(ss, TweenInfo.new(0.9, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {Transparency = 0.55, Thickness = 1.8}):Play()
    task.wait(0.9)
  end
end)
local tc = Instance.new("Frame")
tc.Size = UDim2.fromScale(1, 1)
tc.Position = UDim2.fromScale(0, 0)
tc.BackgroundColor3 = Color3.fromRGB(6, 2, 3)
tc.BackgroundTransparency = 0.15
tc.BorderSizePixel = 0
tc.ZIndex = 0
tc.Parent = sq
local td = Instance.new("UIGradient")
td.Color = ColorSequence.new({ColorSequenceKeypoint.new(0, Color3.fromRGB(35, 3, 8)), ColorSequenceKeypoint.new(0.5, Color3.fromRGB(6, 2, 3)), ColorSequenceKeypoint.new(1, Color3.fromRGB(50, 4, 10))})
td.Rotation = 25
td.Parent = tc
local te = Random.new(4421)
for tf = 1, 26 do
  local tg = Instance.new("Frame")
  local th = te:NextInteger(3, 14)
  tg.Size = UDim2.fromOffset(th, math.max(3, math.floor((th * te:NextNumber(0.45, 1.5)))))
  tg.Position = UDim2.fromScale(te:NextNumber(0.02, 0.98), te:NextNumber(0.03, 0.97))
  tg.Rotation = te:NextInteger(0, 360)
  tg.BackgroundColor3 = Color3.fromRGB(te:NextInteger(100, 220), 5, 14)
  tg.BackgroundTransparency = te:NextNumber(0.05, 0.35)
  tg.BorderSizePixel = 0
  tg.ZIndex = 0
  tg.Parent = tc
  local ti = Instance.new("UICorner")
  ti.CornerRadius = UDim.new(0.5, 0)
  ti.Parent = tg
end
task.spawn(function()
  while ((sq and sq.Parent) and not sn) do
    local tj = Instance.new("Frame")
    tj.Size = UDim2.fromOffset(te:NextInteger(3, 6), te:NextInteger(6, 12))
    tj.Position = UDim2.new(te:NextNumber(0.02, 0.98), 0, 0, -20)
    tj.BackgroundColor3 = Color3.fromRGB(te:NextInteger(160, 230), 5, 15)
    tj.BackgroundTransparency = 0.15
    tj.BorderSizePixel = 0
    tj.ZIndex = 1
    tj.Parent = tc
    local tk = Instance.new("UICorner")
    tk.CornerRadius = UDim.new(0.5, 0)
    tk.Parent = tj
    a:Create(tj, TweenInfo.new(te:NextNumber(2, 4), Enum.EasingStyle.Linear), {Position = UDim2.new(tj.Position.X.Scale, 0, 1, 20)}):Play()
    task.delay(4.2, function()
      if tj then
        tj:Destroy()
      end
    end)
    task.wait(te:NextNumber(0.8, 2.4))
  end
end)
local tl = Instance.new("Frame")
tl.Size = UDim2.new(1, 0, 0, 90)
tl.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
tl.BorderSizePixel = 0
tl.ZIndex = 1
tl.Parent = sq
local tm = Instance.new("UIGradient")
tm.Transparency = NumberSequence.new({NumberSequenceKeypoint.new(0, 0.94), NumberSequenceKeypoint.new(0.4, 0.98), NumberSequenceKeypoint.new(1, 1)})
tm.Rotation = 90
tm.Parent = tl
local tn = Instance.new("Frame")
tn.Size = UDim2.new(0, 60, 0, 3)
tn.AnchorPoint = Vector2.new(0.5, 0)
tn.Position = UDim2.new(0.5, 0, 0, 0)
tn.BackgroundColor3 = ao.BloodBright
tn.BorderSizePixel = 0
tn.ZIndex = 2
tn.Parent = sq
local to = Instance.new("UICorner")
to.CornerRadius = UDim.new(1, 0)
to.Parent = tn
local tp = Instance.new("TextLabel")
tp.Size = UDim2.new(0, 44, 0, 44)
tp.AnchorPoint = Vector2.new(0.5, 0)
tp.Position = UDim2.new(0.5, 0, 0, 22)
tp.BackgroundColor3 = ao.CardGlass
...
tp.ZIndex = 2
tp.Parent = sq
local tq = Instance.new("UICorner")
tq.CornerRadius = UDim.new(0, 12)
tq.Parent = tp
local tr = Instance.new("UIStroke")
tr.Color = ao.BloodBright
tr.Transparency = 0.35
tr.Thickness = 1.3
tr.Parent = tp
task.spawn(function()
  while ((tp and tp.Parent) and not sn) do
    a:Create(tr, TweenInfo.new(0.7, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {Transparency = 0.05, Thickness = 2}):Play()
    task.wait(0.7)
    a:Create(tr, TweenInfo.new(0.7, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {Transparency = 0.55, Thickness = 1.3}):Play()
    task.wait(0.7)
  end
end)
local ts = Instance.new("TextLabel")
ts.Size = UDim2.new(1, 0, 0, 30)
ts.Position = UDim2.new(0, 0, 0, 76)
ts.BackgroundTransparency = 1
ts.Font = Enum.Font.GothamBlack
ts.TextSize = 24
ts.TextColor3 = ao.TextPrimary
ts.RichText = true
ts.Text = "<b>GORE</b> <font color=\"rgb(220,18,30)\">LAB</font>"
ts.TextXAlignment = Enum.TextXAlignment.Center
ts.ZIndex = 2
ts.Parent = sq
local tt = Instance.new("TextLabel")
tt.Size = UDim2.new(1, -40, 0, 16)
tt.AnchorPoint = Vector2.new(0.5, 0)
tt.Position = UDim2.new(0.5, 0, 0, 110)
tt.BackgroundTransparency = 1
tt.Font = Enum.Font.Gotham
tt.TextSize = 12
tt.TextColor3 = ao.TextSecondary
tt.Text = "Enter your access key to continue"
tt.TextXAlignment = Enum.TextXAlignment.Center
tt.ZIndex = 2
tt.Parent = sq
local tu = Instance.new("TextLabel")
tu.Size = UDim2.new(1, -48, 0, 14)
tu.AnchorPoint = Vector2.new(0.5, 0)
tu.Position = UDim2.new(0.5, 0, 0, 145)
tu.BackgroundTransparency = 1
tu.Font = Enum.Font.GothamBold
tu.TextSize = 10
tu.TextColor3 = ao.TextMuted
tu.TextXAlignment = Enum.TextXAlignment.Left
tu.Text = "ACCESS KEY"
tu.ZIndex = 2
tu.Parent = sq
local tv = Instance.new("TextBox")
tv.Size = UDim2.new(1, -48, 0, 42)
tv.AnchorPoint = Vector2.new(0.5, 0)
tv.Position = UDim2.new(0.5, 0, 0, 163)
tv.BackgroundColor3 = Color3.fromRGB(15, 6, 10)
tv.BackgroundTransparency = 0.2
tv.BorderSizePixel = 0
tv.Font = Enum.Font.GothamBold
tv.TextSize = 14
tv.TextColor3 = ao.TextPrimary
tv.PlaceholderText = "enter key…"
tv.PlaceholderColor3 = ao.TextMuted
tv.TextXAlignment = Enum.TextXAlignment.Center
tv.ClearTextOnFocus = false
tv.Text = ""
tv.ZIndex = 2
tv.Parent = sq
local tw = Instance.new("UICorner")
tw.CornerRadius = UDim.new(0, 8)
tw.Parent = tv
local tx = Instance.new("UIStroke")
tx.Color = ao.CardStroke
tx.Transparency = 0.5
tx.Thickness = 1
tx.Parent = tv
tv.Focused:Connect(function()
  a:Create(tx, TweenInfo.new(0.2), {Color = ao.BloodBright, Transparency = 0.2, Thickness = 1.4}):Play()
end)
tv.FocusLost:Connect(function()
  a:Create(tx, TweenInfo.new(0.2), {Color = ao.CardStroke, Transparency = 0.5, Thickness = 1}):Play()
end)
local ty = Instance.new("TextLabel")
ty.Size = UDim2.new(1, -48, 0, 16)
ty.AnchorPoint = Vector2.new(0.5, 0)
ty.Position = UDim2.new(0.5, 0, 0, 212)
ty.BackgroundTransparency = 1
ty.Font = Enum.Font.GothamMedium
ty.TextSize = 11
ty.TextColor3 = ao.TextSecondary
ty.TextXAlignment = Enum.TextXAlignment.Center
ty.Text = "Awaiting key…"
ty.ZIndex = 2
ty.Parent = sq
local tz = Instance.new("TextButton")
tz.Size = UDim2.new(1, -48, 0, 44)
tz.AnchorPoint = Vector2.new(0.5, 0)
tz.Position = UDim2.new(0.5, 0, 0, 240)
tz.BackgroundColor3 = ao.BloodBright
tz.BorderSizePixel = 0
tz.AutoButtonColor = false
tz.Font = Enum.Font.GothamBold
tz.TextSize = 14
tz.TextColor3 = Color3.fromRGB(255, 240, 240)
tz.Text = "UNLOCK"
tz.ZIndex = 2
tz.Parent = sq
local ua = Instance.new("UICorner")
ua.CornerRadius = UDim.new(0, 10)
ua.Parent = tz
local ub = Instance.new("UIStroke")
ub.Color = ao.BloodGlow
ub.Transparency = 0.4
ub.Thickness = 1.4
ub.Parent = tz
tz.MouseEnter:Connect(function()
  a:Create(tz, TweenInfo.new(0.15), {BackgroundColor3 = ao.BloodGlow}):Play()
end)
tz.MouseLeave:Connect(function()
  a:Create(tz, TweenInfo.new(0.15), {BackgroundColor3 = ao.BloodBright}):Play()
end)
local uc = Instance.new("TextButton")
uc.Size = UDim2.new(1, -48, 0, 36)
uc.AnchorPoint = Vector2.new(0.5, 0)
uc.Position = UDim2.new(0.5, 0, 0, 296)
uc.BackgroundColor3 = Color3.fromRGB(24, 10, 14)
uc.BackgroundTransparency = 0.15
uc.BorderSizePixel = 0
uc.AutoButtonColor = false
uc.Font = Enum.Font.GothamBold
uc.TextSize = 12
uc.TextColor3 = ao.TextPrimary
uc.Text = "🔑  GET KEY"
uc.ZIndex = 2
uc.Parent = sq
local ud = Instance.new("UICorner")
ud.CornerRadius = UDim.new(0, 10)
ud.Parent = uc
local ue = Instance.new("UIStroke")
ue.Color = ao.BloodBright
ue.Transparency = 0.55
ue.Thickness = 1.2
ue.Parent = uc
uc.MouseEnter:Connect(function()
  a:Create(uc, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(38, 14, 20), BackgroundTransparency = 0}):Play()
  a:Create(ue, TweenInfo.new(0.15), {Color = ao.BloodGlow, Transparency = 0.25}):Play()
end)
uc.MouseLeave:Connect(function()
  a:Create(uc, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(24, 10, 14), BackgroundTransparency = 0.15}):Play()
  a:Create(ue, TweenInfo.new(0.15), {Color = ao.BloodBright, Transparency = 0.55}):Play()
end)
uc.MouseButton1Click:Connect(function()
  aq(ap.Click, 0.4)
  local uf = "https://discord.gg/NErP8zzq2"
  local ug = pcall(function()
    if setclipboard then
      setclipboard(uf)
    end
  end)
  if ug then
    ty.Text = "Discord link copied to clipboard!"
    ty.TextColor3 = ao.Success
    uc.Text = "✅  COPIED"
    task.wait(1.5)
    ...
    ty.Text = "Couldn't copy — link: discord.gg/NErP8zzq2"
    ty.TextColor3 = ao.Warning
  end
end)
local uh = Instance.new("TextLabel")
uh.Size = UDim2.new(1, 0, 0, 18)
uh.AnchorPoint = Vector2.new(0.5, 1)
uh.Position = UDim2.new(0.5, 0, 1, -12)
uh.BackgroundTransparency = 1
uh.Font = Enum.Font.GothamMedium
uh.TextSize = 10
uh.TextColor3 = ao.TextMuted
uh.Text = "GORE LAB • OFFLINE GATE"
...
local function uj()
  ...
  local uk = tv.Text:gsub("%s+", "")
  if (#uk == 0) then
    ty.Text = "Enter a key first"
    ty.TextColor3 = ao.Warning
    aq(ap.Notif, 0.3)
    return
  end
  ...
  task.spawn(function()
    ...
    if (uk:lower() == sm) then
      sn = true
      ty.Text = "Access granted"
      ty.TextColor3 = ao.Success
      aq(ap.Toggle, 0.5)
      for ul, um in ipairs(sp:GetDescendants()) do
        if um:IsA("GuiObject") then
          a:Create(um, TweenInfo.new(0.35), {BackgroundTransparency = 1}):Play()
        end
        if ((um:IsA("TextLabel") or um:IsA("TextButton")) or um:IsA("TextBox")) then
          a:Create(um, TweenInfo.new(0.3), {TextTransparency = 1}):Play()
        end
        if um:IsA("UIStroke") then
          a:Create(um, TweenInfo.new(0.3), {Transparency = 1}):Play()
        end
      end
      task.wait(0.4)
      sp:Destroy()
      ...
      tz.Active = true
      ty.Text = "Invalid key"
      ty.TextColor3 = ao.Danger
      aq(ap.Notif, 0.5)
    end
  end)
end
tz.MouseButton1Click:Connect(function()
  aq(ap.Click, 0.4)
  uj()
end)
tv.FocusLost:Connect(function(un)
  if un then
    uj()
  end
end)
